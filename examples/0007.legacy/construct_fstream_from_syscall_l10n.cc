...
*/
	...
	*fout.rdbuf() = std::move(*fbf.fb);
	...
	println(imbue(l10n, fast_io::c_stdout()), "Unix Timestamp:", unix_ts,
			...
			"\n"
			"UTC era_d_t_fmt:",
			fast_io::mnp::era_d_t_fmt(utc(unix_ts)),
			"\n"
			"Local iso8601:",
			local(unix_ts),
			"\n"
			"Local date_fmt:",
			fast_io::mnp::date_fmt(local(unix_ts)),
			"\n"
			"Local era_d_t_fmt:",
			fast_io::mnp::era_d_t_fmt(local(unix_ts)),
			"\n"
			"Local Timezone:",
...
D:\hg\fast_io\examples\0007.legacy>g++ -o construct_fstream_from_syscall_l10n construct_fstream_from_syscall_l10n.cc
-Ofast -std=c++23 -s -flto -march=native -I../../include -lntdll
