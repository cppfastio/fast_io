...
int main(int argc, char **argv)
{
	...
	println("counts of rust source files (*.rs): ", rs_count,
			"\n"
			"counts of C++ source files (*.cc,*.cpp,*.cxx): ",
			cpp_count,
			"\n"
			"counts of C source files (*.c): ",
			c_count,
			"\n"
			"counts of C / C++ common header files (*.h): ",
			h_count,
			"\n"
			"counts of C++ header files (*.hpp): ",
			hpp_count,
			"\n"
			"counts of all C++ files: ",
			cpp_count + h_count + hpp_count,
			"\n"
			"counts of all C/C++ files: ",
			c_count + cpp_count + h_count + hpp_count,
			"\n"
			"counts of all Objective C/C++: ",
			mm_count,
			"\n"
			"counts of assembly files: ",
			assembly_count,
			"\n"
			"counts of python files:",
			py_count);
}