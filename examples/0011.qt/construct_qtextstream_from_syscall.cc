...
*/
	...
	println("Unix Timestamp:", unix_ts,
			...
			"\n"
			"fd:",
			static_cast<fast_io::posix_io_observer>(qf).fd
#if (defined(_WIN32) && !defined(__WINE__)) || defined(__CYGWIN__)
			,
			"\n"
			"win32 HANDLE:",
			static_cast<fast_io::win32_io_observer>(qf).handle
#ifndef _WIN32_WINDOWS
			// NT kernel
			,
			"\n"
			"zw HANDLE:",
			static_cast<fast_io::zw_io_observer>(qf).handle,
			"\n"
			"nt HANDLE:",
			static_cast<fast_io::nt_io_observer>(qf).handle
#endif
#endif
	);
}
