...
template <typename T> inline constexpr generator_iterator<T> &operator++(generator_iterator<T> &b)
{
    b.handle.resume();
#ifdef __cpp_exceptions
    if (b.handle.done())
...
template <typename T> inline constexpr void operator++(generator_iterator<T> &b, int)
{
    b.handle.resume();
#ifdef __cpp_exceptions
    if (b.handle.done())
...
template <typename T> inline constexpr details::generator_iterator<T> begin(generator<T> &gen)
{
    gen.handle.resume();
#ifdef __cpp_exceptions
    if (gen.handle.done())
...
template <typename T> inline constexpr ::std::default_sentinel_t end(generator<T> &gen)
{
    return {};
}
template <typename T> inline constexpr details::generator_iterator<T> cbegin(generator<T> const &gen)
{
    gen.handle.resume();
#ifdef __cpp_exceptions
    if (gen.handle.done())
...
template <typename T> inline constexpr ::std::default_sentinel_t cend(generator<T> const &gen)
{
    return {};
}
template <typename T> inline constexpr details::generator_iterator<T> begin(generator<T> const &gen)
{
    gen.handle.resume();
#ifdef __cpp_exceptions
    if (gen.handle.done())
...
template <typename T> inline constexpr ::std::default_sentinel_t end(generator<T> const &gen)
{
    return {};
}
...
namespace details
{
template <typename T> inline generator<T> generator_promise<T>::get_return_object() noexcept
{
