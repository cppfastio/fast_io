...
inline auto nt_api_common(T const& t,Func callback)
{
	...
	else	//types like std::basic_string_view, we must allocate new space to hold that type
	{
		...
		{
			return ::fast_io::details::api_common_copy_append0_strvw_case_impl<allocator_type,wchar_t>(reinterpret_cast<wchar_t_const_may_alias_ptr>(t.data()),t.length(),callback);
		}
		else
		{
