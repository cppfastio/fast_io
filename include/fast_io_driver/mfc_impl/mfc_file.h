...
template <::std::integral T> class basic_mfc_io_observer
{
  public:
    using char_type = T;
...
inline void mfc_write_impl(CFile *cfp, void const *first, void const *last)
{
    ...
    mfc_write_n_impl(cfp, reinterpret_cast<::std::byte const *>(first_ptr),
...
template <::std::integral T> inline void scatter_write(basic_mfc_io_observer<T> hd, io_scatters_t scatters)
{
    ::fast_io::details::mfc_scatter_write_impl(hd.phandle, scatters.base, scatters.len);
}

template <::std::integral T> inline void flush(basic_mfc_io_observer<T> hd)
{
    hd.phandle->Flush();
}
...
template <::std::integral ch_type> class basic_mfc_file : public basic_mfc_io_observer<ch_type>
{
  public:
