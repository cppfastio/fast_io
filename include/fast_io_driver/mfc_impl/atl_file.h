...
template <::std::integral T> class basic_atl_io_observer
{
  public:
    using char_type = T;
...
template <::std::integral T> inline void scatter_write(basic_atl_io_observer<T> hd, io_scatters_t scatters)
{
    ::fast_io::details::atl_scatter_write_impl(hd.phandle, scatters.base, scatters.len);
}

template <::std::integral T> inline void flush(basic_atl_io_observer<T> hd)
{
    hd.phandle->Flush();
}
...
template <::std::integral ch_type> class basic_atl_file : public basic_atl_io_observer<ch_type>
{
  public:
