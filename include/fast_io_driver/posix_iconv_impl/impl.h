...
struct iconv_deco_t
{
	...
	char buffer[32]{};
	::std::size_t len{};
	iconv_deco_t(posix_iconv_io_observer piiob) noexcept
		: cd(piiob.cd)
	{}
};
...
inline iconv_deco_from_chars_result iconv_do_impl(iconv_t cd, char const *from_first2, char const *from_last2,
												  char *to_first, char *to_last) noexcept
{
	...
	while (from_first != from_last)
	{
		...
		::std::size_t ret{iconv(cd, __builtin_addressof(from_first), __builtin_addressof(inbytes_left),
								__builtin_addressof(to_first), __builtin_addressof(outbytes_left))};
...
inline ::std::size_t iconv_print_reserve_define_impl(iconv_t cd, char const *inbyteptr, ::std::size_t inbytesize,
...
inline char *iconv_dst_impl(iconv_deco_t &icdt, char const *first, char const *last, char *dst_first,
							char *dst_last) noexcept
{
	...
	if (len)
	{
		...
		auto [src_it, dst_it] = iconv_do_impl(icdt.cd, icdt.buffer, buffer_total_ed, dst_first, dst_last);
...
inline ::std::size_t iconv_deco_reserve_define_impl(iconv_deco_t &icdt, char const *first, ::std::size_t insize,
													char *outbyteptr, ::std::size_t size)
{
...
inline constexpr ::std::size_t deco_reserve_size(io_reserve_type_t<to_char_type, iconv_deco_t>, iconv_deco_t &icdt,
												 ::std::size_t size) noexcept
{
	constexpr ::std::size_t factor{8 / sizeof(to_char_type)};
	return details::intrinsics::mul_or_overflow_die(details::intrinsics::add_or_overflow_die(size, 32), factor);
}

template <::std::contiguous_iterator fromIter, ::std::contiguous_iterator toIter>
inline toIter deco_reserve_define(io_reserve_type_t<::std::iter_value_t<toIter>, iconv_deco_t>, iconv_deco_t &icdt,
								  fromIter first, fromIter last, toIter iter)
{
	...
	return iter + details::iconv_deco_reserve_define_impl(
					  icdt, reinterpret_cast<char const *>(::std::to_address(first)), inbytes,
					  reinterpret_cast<char *>(::std::to_address(iter)), allocated_bytes) /
					  sizeof(to_char_type);
}
...
public:
	...
	constexpr posix_iconv_file(posix_iconv_file &&__restrict other) noexcept
		: posix_iconv_io_observer{other.release()}
	{
	}
...
constexpr iconv_code_cvt_t<io_scatter_t> iconv_code_cvt(posix_iconv_io_observer piiob, rg &&t)
{
	if constexpr (::std::is_array_v<::std::remove_cvref_t<rg>>)
	{
		return {piiob.cd,
				{::std::ranges::data(t),
				 sizeof(::std::ranges::range_value_t<::std::remove_cvref_t<rg>>) * (::std::ranges::size(t) - 1)}};
	}
	else
	{
		return {piiob.cd,
				{::std::ranges::data(t),
				 sizeof(::std::ranges::range_value_t<::std::remove_cvref_t<rg>>) * ::std::ranges::size(t)}};
	}
}
...
inline Iter print_reserve_define(io_reserve_type_t<char_type, iconv_code_cvt_t<io_scatter_t>>, Iter iter,
								 iconv_code_cvt_t<io_scatter_t> v) noexcept
{
	::std::size_t const sz{::fast_io::details::iconv_print_reserve_define_impl(
		v.cd, reinterpret_cast<char const *>(::std::to_address(v.reference.base)), v.reference.len,
