...
template <::std::integral ch_type> class basic_qt_file : public basic_qt_io_observer<ch_type>
{
  public:
    ...
    basic_qt_file(basic_c_family_file<family, ch_type> &&cioh, open_mode mode)
        : basic_qt_io_observer<char_type>{::fast_io::details::open_qfile_with_fp(cioh.fp, mode)}
    {
        cioh.fp = nullptr;
    }
#if !defined(__AVR__)
    basic_qt_file(basic_posix_file<ch_type> &&pioh, open_mode mode)
        : basic_qt_io_observer<char_type>{::fast_io::details::open_qfile_with_fd(pioh.fd, mode)}
    {
        pioh.fd = -1;
    }
  ...
  public:
    constexpr basic_qt_file(basic_qt_file &&__restrict other) noexcept : basic_qt_io_observer<ch_type>{other.release()}
    {
    }
