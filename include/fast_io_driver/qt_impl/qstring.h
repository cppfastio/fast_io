...
template <typename T>
concept qt_qstring_view_like_impl =
    ::std::same_as<::std::remove_cvref_t<T>, QString> || ::std::same_as<::std::remove_cvref_t<T>, qt_error>;
...
constexpr ::std::conditional_t<
    sizeof(char_type) == sizeof(char16_t), basic_io_scatter_t<char_type>,
    manipulators::code_cvt_t<encoding_scheme::execution_charset, encoding_scheme::execution_charset, char16_t>>
status_io_print_forward(io_alias_type_t<char_type>, T const &hstr) noexcept
{
...
inline QString strlike_construct_define(io_strlike_type_t<char16_t, QString>, char16_t const *first,
                                        char16_t const *last)
{
...
template <typename... Args> inline QString u16concat_qt_qstring(Args &&...args)
{
    return ::fast_io::basic_general_concat<false, char16_t, QString>(::std::forward<Args>(args)...);
}

template <typename... Args> inline QString u16concatln_qt_qstring(Args &&...args)
{
    return ::fast_io::basic_general_concat<true, char16_t, QString>(::std::forward<Args>(args)...);
}

inline constexpr io_strlike_reference_wrapper<char16_t, QString> io_strlike_ref(::fast_io::io_alias_t,
                                                                                QString &hstr) noexcept
{
...
template <typename T> inline auto qt_as_filename(T const &hstr) noexcept
{
    ...
    {
        return ::fast_io::manipulators::basic_os_c_str_with_known_size<char16_t>{
            reinterpret_cast<char16_may_alias_const_ptr>(hstr.data()), static_cast<::std::size_t>(hstr.size())};
    }
    else
    {
        return ::fast_io::manipulators::basic_os_str_known_size_without_null_terminated<char16_t>{
            reinterpret_cast<char16_may_alias_const_ptr>(hstr.data()), static_cast<::std::size_t>(hstr.size())};
    }
}
