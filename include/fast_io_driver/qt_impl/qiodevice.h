...
template <::std::integral ch_type, typename T> class basic_general_qdevice_io_observer
{
  public:
    ...
    inline operator basic_posix_io_observer<char_type>()
        requires requires() {
    ...
    inline operator basic_win32_io_observer<char_type>()
        requires requires() {
            { this->qdevice->handle() } -> ::std::same_as<int>;
        }
    {
    ...
    inline operator basic_nt_family_io_observer<family, char_type>()
        requires requires() {
            { this->qdevice->handle() } -> ::std::same_as<int>;
        }
    {
...
template <::std::integral char_type> using basic_qt_io_observer = basic_general_qdevice_io_observer<char_type, QFile>;
