...
inline constexpr win32_memory_page_protect &operator&=(win32_memory_page_protect &x,
                                                       win32_memory_page_protect y) noexcept
{
    return x = x & y;
}
...
template <::std::integral ch_type> class basic_win32_memory_io_observer
{
  public:
...
template <::std::integral ch_type> class basic_win32_memory_io_handle : public basic_win32_memory_io_observer<ch_type>
{
  public:
    ...
    basic_win32_memory_io_handle(basic_win32_memory_io_handle const &other)
    {
        ...
        if (!win32::DuplicateHandle(current_process, other.native_handle(), current_process,
                                    __builtin_addressof(this->native_handle()), 0, true, 2 /*DUPLICATE_SAME_ACCESS*/))
            throw_win32_error();
    ...
    basic_win32_memory_io_handle &operator=(basic_win32_memory_io_handle const &other)
    {
        ...
        void *new_handle{};
        if (!win32::DuplicateHandle(current_process, other.native_handle(), current_process,
                                    __builtin_addressof(new_handle), 0, true, 2 /*DUPLICATE_SAME_ACCESS*/))
            throw_win32_error();
    ...
    constexpr basic_win32_memory_io_handle(basic_win32_memory_io_handle &&__restrict other) noexcept
        : basic_win32_memory_io_observer<ch_type>(other.native_handle(), other.base_address())
    {
        other.native_handle() = nullptr;
        other.base_address() = {};
    }
...
inline constexpr win32_desired_access &operator&=(win32_desired_access &x, win32_desired_access y) noexcept
{
    return x = x & y;
}
...
template <::std::integral ch_type> class basic_win32_memory_file : public basic_win32_memory_io_handle<ch_type>
{
  public:
    using native_handle_type = basic_win32_memory_io_observer<ch_type>::native_handle_type;
    using base_address_type = basic_win32_memory_io_observer<ch_type>::base_address_type;
...
{
    ::std::size_t readed{};
    for (::std::byte ch{}; !win32::ReadProcessMemory(iob.handle, bit_cast<void const *>(iob.base_addr),
                                                     __builtin_addressof(ch), 1, __builtin_addressof(readed));
         ++iob.base_addr)
        ;
    return iob;
}
                                                       ...
                                                       ::std::size_t size, win32_memory_page_protect new_protect)
{
    ...
    if (!win32::VirtualProtectEx(iob.handle, bit_cast<void const *>(iob.base_addr), size,
                                 static_cast<::std::uint_least32_t>(new_protect), __builtin_addressof(old_protect)))
        throw_win32_error();
...
class win32_virtual_protect_guard
{
  ...
  public:
    ...
    ~win32_virtual_protect_guard()
    {
        ...
        win32::VirtualProtectEx(process_handle, bit_cast<void const *>(address), region_size,
                                static_cast<::std::uint_least32_t>(oprotect), __builtin_addressof(useless));
    }
};
