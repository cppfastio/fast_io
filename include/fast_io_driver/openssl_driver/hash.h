...
template <typename metainfotype> struct basic_ossl_hash_context
{
    ...
    {
        if (meta_info_type::update_function(__builtin_addressof(handle), first,
                                            static_cast<::std::size_t>(last - first)) == 0) [[unlikely]]
            ::fast_io::fast_terminate();
    }
    void do_final() noexcept
    {
        if (meta_info_type::final_function(reinterpret_cast<char unsigned *>(digest_buffer),
                                           __builtin_addressof(handle)) == 0) [[unlikely]]
            ::fast_io::fast_terminate();
    }
};
