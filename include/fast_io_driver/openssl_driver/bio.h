...
struct bio_stm_scope_ptr
{
	...
	explicit constexpr bio_stm_scope_ptr(stream *p) noexcept
		: s{p}
	{}
...
public:
	...
	explicit constexpr basic_bio_file(native_hd bio) noexcept
		: basic_bio_io_observer<char_type>{bio}
	{
	}
	...
	constexpr basic_bio_file(basic_bio_file &&__restrict bf) noexcept
		: basic_bio_io_observer<char_type>(bf.bio)
	{
		bf.bio = nullptr;
	}
