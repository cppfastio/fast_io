...
public:
	...
	{}
	constexpr win32_internet_handle(void *value)
		: handle(value)
	{}

	constexpr win32_internet_handle(win32_internet_handle &&__restrict bmv) noexcept
		: handle(bmv.handle)
	{
		bmv.handle = {};
	}
...
template <typename... Args>
	requires requires(Args &&...args) { InternetOpenA(::std::forward<Args>(args)...); }
...
public:
	...
	basic_win32_internet_https_client(cstring_view host, cstring_view method, cstring_view object_name,
									  ::std::uint_least32_t port = 443)
		: session(
			  win32_internet_open("Microsoft Internet Explorer", INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0)),
		  connection(win32_internet_connect(session, host.data(), port, nullptr, nullptr, INTERNET_SERVICE_HTTP, 0, 0)),
		  request(win32_http_open_request(
			  connection, method.data(), object_name.data(), HTTP_VERSION, nullptr, nullptr,
