...
public:
	...
	explicit operator basic_posix_io_observer<char_type>() const noexcept
		requires(::std::derived_from<T, ::llvm::raw_fd_ostream>)
	{
...
inline constexpr basic_general_raw_ostream_io_observer<char_type, T>
io_value_handle(basic_general_raw_ostream_io_observer<char_type, T> osiob) noexcept
{
	return osiob;
}
...
inline void llvm_ostream_write_impl(::llvm::raw_ostream *os, void const *first_vptr, void const *last_vptr)
{
	...
	{
		::fast_io::llvm::details::llvm_raw_ostream_set_obuffer_curr_ptr(
			os, ::fast_io::details::non_overlapped_copy_n(first, n, os_obuffer_curr_ptr));
		return;
	}
	os->write(first, n);
}
