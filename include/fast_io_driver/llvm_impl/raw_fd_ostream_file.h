																	 ...
																	 ::llvm::raw_fd_ostream *otherfdos)
{
	...
	hack_set_fd_to_llvm_raw_fd_ostream(thisfdos,
									   ::fast_io::details::sys_dup2(hack_fd_from_llvm_raw_fd_ostream(otherfdos),
																	hack_fd_from_llvm_raw_fd_ostream(thisfdos)));
	return thisfdos;
}
...
public:
	...
	explicit constexpr basic_raw_fd_ostream_file(T *p) noexcept
		: basic_raw_fd_ostream_io_observer<char_type>{p}
	{
	}
	constexpr basic_raw_fd_ostream_file(basic_raw_fd_ostream_file &&__restrict other) noexcept
		: basic_raw_fd_ostream_io_observer<char_type>{other.os}
	{
		other.os = nullptr;
	}
