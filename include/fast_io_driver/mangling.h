...
struct cxa_demangle
{
	...
	explicit cxa_demangle(char const *mangled_name) noexcept
	{
		...
		buffer = static_cast<char *>(
			abi::__cxa_demangle(mangled_name, nullptr, __builtin_addressof(length), __builtin_addressof(status)));
		if (buffer != nullptr)
		{
	...
	cxa_demangle(cxa_demangle const &other) noexcept
		: buffer(reinterpret_cast<char *>(malloc(other.length + 1))), length(other.length)
	{
		if (buffer == nullptr)
		{
			fast_terminate();
	...
	constexpr cxa_demangle(cxa_demangle &&__restrict other) noexcept
