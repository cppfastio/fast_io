...
public:
	...
	explicit constexpr cs_file(T cs) noexcept
		: cs_io_observer{cs}
	{
	}
	cs_file(cs_file const &) = delete;
	cs_file &operator=(cs_file const &) = delete;
	constexpr cs_file(cs_file &&__restrict other) noexcept
		: cs_io_observer{other.csh}
	{
		other.csh = 0;
	}
...
public:
	...
	cs_insn_range(cs_insn_range &&__restrict other) noexcept
		: ins{other.ins}, count{other.count}
	{
		other.ins = nullptr;
		other.count = 0;
	}
