...
{
template <::std::size_t wd = 80, ::std::size_t ht = 25> struct terminal
{
...
template <::std::size_t wd = 80, ::std::size_t ht = 25> inline void next_line(terminal<wd, ht> &tem) noexcept
{
    if (++tem.row == ht) [[unlikely]]
        tem.row = {};
    clear_line(tem);
}
template <::std::size_t wd = 80, ::std::size_t ht = 25> inline void next_character(terminal<wd, ht> &tem) noexcept
{
...
template <::std::size_t wd, ::std::size_t ht> inline void put(terminal<wd, ht> &tem, char ch) noexcept
{
    auto ptr = (char volatile *)0xb8000;
    switch (ch)
    {
