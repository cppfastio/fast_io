...
template <::std::integral ch_type>
	requires(sizeof(ch_type) == 1)
inline constexpr basic_avio_context_io_observer<ch_type>
io_value_handle(basic_avio_context_io_observer<ch_type> baciob) noexcept
{
	return baciob;
}
...
public:
	...
	explicit constexpr basic_avio_context_file(::AVIOContext *ctxx) noexcept
		: basic_avio_context_io_observer<ch_type>{ctxx}
	{
	}
	void close() noexcept
	{
	...
	basic_avio_context_file(basic_avio_context_file &&other) noexcept
		: basic_avio_context_io_observer<ch_type>{other.avios}
	{
		other.avios = nullptr;
	}
...
struct avio_buffer
{
	...
	explicit constexpr avio_buffer(void *bf) noexcept
		: buffer(bf)
	{}
	void close() noexcept
	{
...
inline basic_avio_buffer_context<typename rftype::char_type> create_avio_context_impl(rftype rf)
{
	...
	ctx = ::avio_alloc_context(
		reinterpret_cast<char unsigned *>(bfptr), buffersize, AVIO_FLAG_WRITE, ptr,
...
struct avformat_context_guard
{
	...
	explicit constexpr avformat_context_guard(::AVFormatContext *p) noexcept
		: pFormatCtx{p}
	{}
...
struct avformat_input_guard
{
	...
	explicit constexpr avformat_input_guard(::AVFormatContext **pp) noexcept
		: ppFormatCtx{pp}
	{}
