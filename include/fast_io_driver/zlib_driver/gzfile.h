...
{
    int fdn;
    ::fast_io::details::my_memcpy(__builtin_addressof(fdn),
                                  reinterpret_cast<::std::byte const *>(gzfile) + offsetof(gz_state_model, fd),
                                  sizeof(int));
    return fdn;
}
/*
...
{
        char* ptr;
        ::fast_io::details::my_memcpy(__builtin_addressof(ptr),reinterpret_cast<::std::byte
const*>(gzfile)+offsetof(gz_state_model,in),sizeof(char *)); return ptr;
}
inline char* hack_gz_file_out(gzFile gzfile)
{
        char* ptr;
        ::fast_io::details::my_memcpy(__builtin_addressof(ptr),reinterpret_cast<::std::byte
const*>(gzfile)+offsetof(gz_state_model,out),sizeof(char *)); return ptr;
}
*/
} // namespace details

template <::std::integral ch_type> class basic_gz_io_observer
{
  public:
...
template <::std::integral ch_type> class basic_gz_file : public basic_gz_io_observer<ch_type>
{
  public:
    using native_handle_type = gzFile;
    using char_type = ch_type;
    ...
    constexpr basic_gz_file(basic_gz_file &&__restrict bmv) noexcept
        : basic_gz_io_observer<char_type>(bmv.native_handle())
    {
...
template <::std::integral char_type> inline void flush(basic_gz_io_observer<char_type> giob)
{
...
template <::std::integral char_type> using basic_ibuf_gz_file = basic_ibuf<basic_gz_file<char_type>>;
