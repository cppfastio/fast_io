...
public:
	...
	{
		int err{};
		this->s=::fast_io::noexcept_call(pa_simple_new,server,name,dir,dev,stream_name,ss,map,attr,__builtin_addressof(err));
		if(this->s==nullptr)
	...
	constexpr basic_pulseaudio_simple_file(basic_pulseaudio_simple_file&& __restrict hd) noexcept:basic_pulseaudio_simple_file(hd.s)
	{
		hd.s=nullptr;
	}
...
{
	int err{};
	if(::fast_io::noexcept_call(pa_simple_read,ps,first,static_cast<::std::size_t>(last-first),__builtin_addressof(err))<0)[[unlikely]]
		throw_pulse_audio_error(err);
}
...
{
	int err{};
	if(::fast_io::noexcept_call(pa_simple_write,ps,first,static_cast<::std::size_t>(last-first),__builtin_addressof(err))<0)[[unlikely]]
		throw_pulse_audio_error(err);
}
