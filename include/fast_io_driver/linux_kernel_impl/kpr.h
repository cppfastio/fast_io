...
template <::std::integral ch_type> struct basic_kpr
{
    using char_type = ch_type;
    kern level{kern::default_value};
};

template <bool line> inline constexpr char8_t const *kern_to_fmt(kern k) noexcept
{
    if constexpr (line)
    {
        switch (k)
...
template <bool line> inline void linux_kpr_raw_write(::fast_io::kern k, void const *start, void const *last) noexcept
{
...
inline constexpr auto linux_kernel_printk_formatter_cal() noexcept
{
    ...
    ::fast_io::freestanding::array<char8_t,
                                   (k == kern::default_value ? 0 : 2) + n * 4 + 1 + static_cast<::std::size_t>(line)>
        fmts;
...
template <::std::size_t n, bool line> inline constexpr char8_t const *cal_kpr_fmt(kern k) noexcept
{
    switch (k)
    {
        case kern::emerg:
