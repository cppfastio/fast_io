...
template <::std::integral ch_type, boost_iostreams_any_device src_type> class basic_boost_iostreams
{
  public:
...
{
    auto ret{out_device.source.write(reinterpret_cast<char const *>(::std::to_address(begin)),
...
template <::std::integral ch_type, boost_iostream_seekable_device src_type, ::std::contiguous_iterator Iter, typename T,
          ::std::integral U>
inline auto seek(basic_boost_iostreams<ch_type, src_type> &dev, seek_type_t<T>, U i, seekdir s = seekdir::beg)
{
    return dev.seek(seek_precondition<::std::intmax_t, T, ch_type>(i),
                    static_cast<::std::ios::seekdir>(static_cast<int>(s)));
}
...
template <boost_iostreams_any_device src_type> using boost_iostreams = basic_boost_iostreams<char, src_type>;
template <boost_iostreams_any_device src_type> using u8boost_iostreams = basic_boost_iostreams<char8_t, src_type>;
template <boost_iostreams_any_device src_type> using wboost_iostreams = basic_boost_iostreams<wchar_t, src_type>;
} // namespace fast_io
