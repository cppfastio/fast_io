...
template <bool dec> inline void py_inc_dec_ref_model_impl(void *p)
{
    if constexpr (dec)
    {
        Py_DECREF(p);
...
template <bool dec> inline void py_inc_dec_ref_impl(void *p) noexcept
{
    if (p == nullptr)
        return;
...
inline pyobject_file strlike_construct_define(io_strlike_type_t<char, pyobject_file>, char const *first,
                                              char const *last) noexcept
{
    ::std::size_t diff{static_cast<::std::size_t>(last - first)};
    if constexpr (::std::numeric_limits<Py_ssize_t>::max() < ::std::numeric_limits<::std::size_t>::max())
    {
        constexpr Py_ssize_t max_sz{
            static_cast<Py_ssize_t>(::std::numeric_limits<Py_ssize_t>::max() / static_cast<Py_ssize_t>(sizeof(char)))};
        if (max_sz < diff)
            ::fast_io::fast_terminate();
    }
...
template <typename... Args> inline pyobject_file concat_pyobject_file(Args &&...args)
{
    return ::fast_io::basic_general_concat<false, char, pyobject_file>(::std::forward<Args>(args)...);
}

template <typename... Args> inline pyobject_file concatln_pyobject_file(Args &&...args)
{
...
inline ::std::conditional_t<
    sizeof(char_type) == sizeof(char), basic_io_scatter_t<char_type>,
    manipulators::code_cvt_t<encoding_scheme::execution_charset, encoding_scheme::execution_charset, char>>
status_io_print_forward(io_alias_type_t<char_type>, pyobject_io_observer piob) noexcept
{
