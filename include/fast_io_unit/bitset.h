...
inline constexpr ::std::size_t print_reserve_size(io_reserve_type_t<char_type, bitset_print_reserve_proxy>,
												  bitset_print_reserve_proxy p) noexcept
{
	return p.bits;
}
...
template <::std::integral char_type>
inline constexpr char_type *prv_rsv_bitset_impl(char_type *iter,
												typename bitset_print_reserve_proxy::native_storage_const_pointer ptr,
												::std::size_t bits) noexcept
{
	using nr_type = typename bitset_print_reserve_proxy::native_storage_type;
	constexpr ::std::size_t bits_per_rep{::std::numeric_limits<nr_type>::digits};
...
template <::std::integral char_type>
inline constexpr char_type *print_reserve_define(io_reserve_type_t<char_type, bitset_print_reserve_proxy>,
												 char_type *iter, bitset_print_reserve_proxy p) noexcept
{
	return ::fast_io::details::prv_rsv_bitset_impl(iter, p.reference, p.bits);
}

template <::std::size_t n>
inline bitset_print_reserve_proxy print_alias_define(io_alias_t, ::std::bitset<n> const &bts) noexcept
{
	return ::fast_io::bitset_print_reserve_proxy{
		reinterpret_cast<::fast_io::bitset_print_reserve_proxy::native_storage_const_pointer>(__builtin_addressof(bts)),
		n};
}
