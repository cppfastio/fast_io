...
inline constexpr char32_t gb18030_ranges[13][4]{{0x0452, 0x2010, linear(0x8130D330), linear(0x8136A531)},
	...
	if (cdpt < 0x110000) [[likely]]
	{
		...
		for (::std::size_t i{}; i != 13; ++i)
		{
			...
			if (static_cast<char32_t>(cdpt - e0) < diff)
			{
				...
				p_dst[3] = static_cast<T>(0x30 + gb % 10);
				gb /= 10;
				p_dst[2] = static_cast<T>(0x81 + gb % 126);
				gb /= 126;
				p_dst[1] = static_cast<T>(0x30 + gb % 10);
				gb /= 10;
...
inline constexpr ::std::size_t get_gb18030_code_units_unhappy_pdstsz(char32_t u32, char *p_dst,
																	 ::std::size_t pdstsz) noexcept
{
	if (u32 < 0x110000) [[likely]]
	{
		...
		for (::std::size_t i{}; i != 13; ++i)
		{
			...
			if (static_cast<char32_t>(u32 - e0) < diff)
			{
				...
				p_dst[3] = static_cast<char>(0x30 + gb % 10);
				gb /= 10;
				p_dst[2] = static_cast<char>(0x81 + gb % 126);
				gb /= 126;
				p_dst[1] = static_cast<char>(0x30 + gb % 10);
				gb /= 10;
