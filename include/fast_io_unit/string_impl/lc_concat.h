	...
	else if constexpr (lc_dynamic_reserve_printable<char_type, T>)
	{
		...
		else
		{
			return ::fast_io::details::intrinsics::add_or_overflow_die(
				res, lc_calculate_scatter_dynamic_reserve_size_with_scatter<char_type>(all, args...));
		}
	}
...
lc_print_scatter_define_extract_one(basic_lc_all<typename T::char_type> const *all, T t)
{
...
inline constexpr T lc_concat_decay_impl(basic_lc_all<typename T::value_type> const *all, Args... args)
{
	...
	if constexpr (((!(lc_dynamic_reserve_printable<ch_type, Args> || lc_scatter_printable<ch_type, Args> ||
					  lc_printable<ostring_ref_type, Args>)) &&
				   ...))
	{
						...
						...))
	{
		...
		else
		{
			::std::size_t total_size{::fast_io::details::intrinsics::add_or_overflow_die(
				sz_with_line, lc_calculate_scatter_dynamic_reserve_size_with_scatter<ch_type>(all, args...))};
			T str;
...
inline constexpr T basic_lc_concat(basic_lc_all<typename T::value_type> const *all, Args... args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<false, T>(
		all, io_print_forward<typename T::value_type>(io_print_alias(args))...);
}
...
inline
	...
	constexpr
#endif
	::std::basic_string<ch_type>
	lc_concat(basic_lc_all<ch_type> const *all, Args... args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<false, ::std::basic_string<ch_type>>(
		all, io_print_forward<ch_type>(io_print_alias(args))...);
}
...
#endif
	::std::basic_string<ch_type>
	lc_concatln(basic_lc_all<ch_type> const *all, Args... args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<true, ch_type>(
		all, io_print_forward<ch_type>(io_print_alias(args))...);
}
...
#endif
	::std::string
	lc_concat(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<false, ::std::string>(
		loc.loc.all, io_print_forward<char>(io_print_alias(args))...);
}
...
#endif
	::std::string
	lc_concatln(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<true, ::std::string>(
		loc.loc.all, io_print_forward<char>(io_print_alias(args))...);
}
...
inline
	...
	wlc_concat(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<false, ::std::basic_string<wchar_t>>(
		loc.loc.wall, io_print_forward<wchar_t>(io_print_alias(args))...);
}
...
#endif
	::std::basic_string<wchar_t>
	wlc_concatln(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<true, ::std::basic_string<wchar_t>>(
		loc.loc.wall, io_print_forward<wchar_t>(io_print_alias(args))...);
}
...
#endif
	::std::u8string
	u8lc_concat(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<false, ::std::u8string>(
		loc.loc.u8all, io_print_forward<char8_t>(io_print_alias(args))...);
}
...
#endif
	::std::u8string
	u8lc_concatln(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<true, ::std::u8string>(
		loc.loc.u8all, io_print_forward<char8_t>(io_print_alias(args))...);
}
...
#endif
	::std::u16string
	u16lc_concat(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<false, ::std::u16string>(
		loc.loc.u16all, io_print_forward<char16_t>(io_print_alias(args))...);
}
...
#endif
	::std::u16string
	u16lc_concatln(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<true, ::std::u16string>(
		loc.loc.u16all, io_print_forward<char16_t>(io_print_alias(args))...);
}
...
#endif
	::std::u32string
	u32lc_concat(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<false, ::std::u32string>(
		loc.loc.u32all, io_print_forward<char32_t>(io_print_alias(args))...);
}
...
#endif
	::std::u32string
	u32lc_concatln(T &loc, Args &&...args)
{
	return ::fast_io::details::decay::lc_concat_decay_impl<true, ::std::u32string>(
		loc.loc.u32all, io_print_forward<char32_t>(io_print_alias(args))...);
}
