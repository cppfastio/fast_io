...
{
    constexpr ::std::size_t mn1{static_cast<::std::size_t>(n - 1u)};
...
template <::std::size_t n> struct libcxx_msvcabi_symbol_name_holder
{
    char8_t const *ptr{};
...
inline constexpr auto compute_libcxx_stdinoutbufname() noexcept
{
    ...
    else
    {
        ...
        Itanium ABI
        */
        constexpr ::std::size_t stdinoutsz{(isoutbuf ? sizeof(u8"11__stdoutbufI") : sizeof(u8"10__stdinbufI")) - 1};
        constexpr ::std::size_t chartypesize{
            (::std::same_as<char_type, char> || ::std::same_as<char_type, wchar_t>) ? 1 : 2};
        constexpr ::std::size_t buffersize{sizeof(u8"NStEE") + stdinoutsz + chartypesize +
