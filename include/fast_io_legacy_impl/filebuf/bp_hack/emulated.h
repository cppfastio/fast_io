...
template <typename T>
inline void ibuffer_set_curr(::fast_io::basic_general_streambuf_io_observer<T> biob,
							 typename T::char_type *ptr) noexcept
{
	/*
	...
	*/
	...
	{
		constexpr ::std::ptrdiff_t mn{static_cast<::std::ptrdiff_t>(::std::numeric_limits<int>::min())},
			mx{static_cast<::std::ptrdiff_t>(::std::numeric_limits<int>::max())};
		if (diff < mn || mx < diff)
		{
			fast_terminate();
		}
...
template <typename T>
inline void obuffer_set_curr(::fast_io::basic_general_streambuf_io_observer<T> biob,
							 typename T::char_type *ptr) noexcept
{
	/*
	Work around bugs in issue 255
