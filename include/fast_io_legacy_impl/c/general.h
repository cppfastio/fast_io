...
namespace details
{
template <::std::integral char_type> inline auto fgetc_unlocked_impl(FILE *fp) noexcept
{
    if constexpr (::std::same_as<char_type, char> || ::std::same_as<char_type, char8_t>)
    {
#if (defined(_WIN32) && !defined(__WINE__))
        return _fgetc_nolock(fp);
#elif (defined(__NEWLIB__) && __GNU_VISIBLE) ||                                                                        \
    (!defined(__NEWLIB__) &&                                                                                           \
     (_POSIX_C_SOURCE >= 199309L || _XOPEN_SOURCE || _POSIX_SOURCE || _BSD_SOURCE || _SVID_SOURCE))
...
template <::std::integral char_type> inline auto ungetc_unlocked_impl(char_type ch, FILE *fp) noexcept
{
    if constexpr (::std::same_as<char_type, char> || ::std::same_as<char_type, char8_t>)
    {
#if defined(_MSC_VER)
