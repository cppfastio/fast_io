...
template <typename T> struct llvmlibc_atomic
{
    using value_type = T;
    static inline constexpr int llvmlibc_atomic_alignment = sizeof(T) > alignof(T) ? sizeof(T) : alignof(T);
    ...
    File
{
    ...
    enum class FileOp : ::std::uint_least8_t
    {
        NONE,
        READ,
        WRITE,
