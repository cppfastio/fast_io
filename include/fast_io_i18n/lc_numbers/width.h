...
inline constexpr ::std::size_t lc_print_reserve_size_width_impl(basic_lc_all<char_type> const *all, T w,
...
inline constexpr char_type *lc_print_reserve_define_width_ch_impl(basic_lc_all<char_type> const *__restrict all,
																  char_type *iter, T t, ::std::size_t wdt,
																  char_type fillch) noexcept
{
	using value_type = ::std::remove_cvref_t<T>;
...
inline constexpr char_type *lc_print_reserve_define_width_impl(basic_lc_all<char_type> const *__restrict all,
...
inline constexpr ::std::size_t print_reserve_size(basic_lc_all<char_type> const *__restrict all,
												  manipulators::width_t<wm, T> w) noexcept
{
...
template <::std::integral char_type, manipulators::scalar_placement wm, typename T>
	requires((lc_dynamic_reserve_printable<char_type, ::std::remove_cvref_t<T>> ||
			  lc_scatter_printable<char_type, ::std::remove_cvref_t<T>>))
inline constexpr ::std::size_t print_reserve_size(basic_lc_all<char_type> const *__restrict all,
												  manipulators::width_ch_t<wm, T, char_type> w) noexcept
{
	if constexpr (::std::is_reference_v<T>)
	{
		return details::lc_print_reserve_size_width_impl<char_type>(all, parameter<T>{w.reference}, w.width);
