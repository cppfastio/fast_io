...
template <::std::integral year_type> inline constexpr bool is_leap_year(year_type year) noexcept
{
...
inline constexpr ::std::int_least64_t day_diff(::std::int_least64_t year_after, ::std::uint_least8_t month_after,
                                               ::std::uint_least8_t day_after, ::std::int_least64_t year_before,
                                               ::std::uint_least8_t month_before,
                                               ::std::uint_least8_t day_before) noexcept
{
    ...
    if (month_after_minus1 > 11u || month_before_minus1 > 11u) [[unlikely]]
        return 0;
    ::std::int_least64_t value{year_after / 4 - year_before / 4 - year_after / 100 + year_before / 100 +
                               year_after / 400 - year_before / 400 + 365 * (year_after - year_before) +
                               month_accum[month_after_minus1] - month_accum[month_before_minus1] + day_after -
                               day_before};
...
inline constexpr bool test_era_started(::std::int_least64_t y0, ::std::uint_least8_t m0, ::std::uint_least8_t d0,
                                       ::std::int_least64_t y1, ::std::uint_least8_t m1,
                                       ::std::uint_least8_t d1) noexcept
{
    if (y0 < y1)
        return true;
    if (y0 == y1)
                      ...
                      ::std::int_least64_t year, lc_month_day_pass_aux month_day) noexcept
{
    ...
    for (; first != last; ++first)
    {
        ...
        if (direction)
        {
            has_era_start = test_era_started(e.start_date_year, e.start_date_month, e.start_date_day, year, month, day);
        }
        else
        {
            has_era_start = test_era_started(year, month, day, e.start_date_year, e.start_date_month, e.start_date_day);
        }
        if (has_era_start)
        {
            ...
            if (end_special == 0)
            {
                ...
                if (direction)
                {
                    has_era_end = test_era_started(year, month, day, e.end_date_year, e.end_date_month, e.end_date_day);
                }
                else
                {
                    has_era_end = test_era_started(e.end_date_year, e.end_date_month, e.end_date_day, year, month, day);
                }
                if (has_era_end)
                    return first;
            }
...
template <::std::integral char_type>
inline constexpr char_type *non_overlapped_copy_to_lowercase(char_type const *__restrict first, char_type const *last,
                                                             char_type *__restrict dest) noexcept
{
    for (; first != last; ++first)
    {
                                              ...
                                              basic_io_scatter_t<char_type> const &format_str) noexcept
{
    ...
    constexpr ::std::size_t uint_least16_reserve_size{
        print_reserve_size(io_reserve_type<char_type, ::std::uint_least16_t>)};
    constexpr ::std::size_t int_least64_reserve_size{
        print_reserve_size(io_reserve_type<char_type, ::std::int_least64_t>)};
    ::std::size_t value{};
    for (char_type const *i{format_str.base}, *end_it{i + format_str.len}; i != end_it; ++i)
    {
        ...
        switch (*p)
        {
            ...
            case char_literal_v<u8'F', char_type>: {
                constexpr ::std::size_t int_least64_reserve_size_plus6{
                    static_cast<::std::size_t>(2) + uint_least8_reserve_size * 2 + int_least64_reserve_size};
...
template <::std::integral char_type> inline constexpr char_type *lc_copy_01_impl(char_type *iter) noexcept
{
...
template <::std::integral char_type> inline constexpr char_type *lc_copy_12_impl(char_type *iter) noexcept
{
    if constexpr (::std::same_as<char_type, char>)
...
template <::std::integral char_type> inline constexpr char_type *lc_copy_52_impl(char_type *iter) noexcept
{
    if constexpr (::std::same_as<char_type, char>)
...
template <::std::integral char_type> inline constexpr char_type *lc_copy_53_impl(char_type *iter) noexcept
{
    if constexpr (::std::same_as<char_type, char>)
    ...
    for (char_type const *i{format_str.base}, *end_it{i + format_str.len}; i != end_it; ++i)
    {
        ...
        switch (first_format_ch)
        {
                    ...
                    case char_literal_v<u8'Y', char_type>: {
                        ...
                        if (era_result != era_scatter_end) [[likely]]
                        {
                            switch (*p)
                            {
                                ...
                                default: {
                                    ::std::int_least64_t real_era_year_offset{era_result->start_date_year -
                                                                              era_result->offset};
                                    ::std::int_least64_t era_year;
                                    if (::fast_io::details::intrinsics::sub_underflow(tsp.year, real_era_year_offset,
                                                                                      era_year)) [[unlikely]]
                                    {
                                        if (omit)
            ...
            case char_literal_v<u8'G', char_type>: {
                ...
                else if (month == 12u && weekday_of_last_day_this_year < 4u &&
                         weekday_of_last_day_this_year + day > 31u)
                    iter = chrono_year_impl(iter, year + 1);
                else
            ...
            case char_literal_v<u8'U', char_type>: {
                iter = print_reserve_integral_define<10>(
                    iter, static_cast<::std::uint_least16_t>(weekday(tsp.year, 1, 1) + day_of_the_year(tsp) - 1u) / 7u);
                break;
            }
            case char_literal_v<u8'v', char_type>: {
                ...
                } weekday_of_1st_day_this_year{day_diff(year, 1, 1, benchmark_year, benchmark_month, benchmark_day) %
                                               my_make_signed_t<::std::size_t>(ndays)};
                ...
                if (month == 1u && weekday_of_1st_day_this_year.u > ndays - first_week + 1u &&
                    weekday_of_1st_day_this_year.u + day < ndays + 2)
                {
                    // calculate the week number of 12/31 of the last year
                    ::std::uint_least64_t weekday_of_12_31{
                        static_cast<::std::uint_least64_t>(weekday_of_1st_day_this_year.u + day_of_the_year(tsp) - 2u) %
                        ndays};
                    auto day_of_12_31{static_cast<::std::uint_least16_t>(is_leap_year(year - 1) ? 366u : 365u)};
                    auto weekday_of_1st_day_last_year{static_cast<::std::uint_least64_t>(
                        (weekday_of_1st_day_this_year.s - day_of_12_31 - 1) % my_make_signed_t<::std::size_t>(ndays) +
                        static_cast<::std::int_least64_t>(ndays) + 1)};
                    ::std::uint_least64_t weeknum{static_cast<::std::uint_least64_t>(day_of_12_31 +
                                                                                     weekday_of_1st_day_last_year -
                                                                                     weekday_of_12_31 - 1u) /
                                                  ndays};
                ...
                else if (month == 12u && weekday_of_last_day_this_year <= ndays - first_week &&
                         weekday_of_last_day_this_year + day >= 32)
                    iter = lc_copy_01_impl(iter);
                else [[likely]]
                {
                    ::std::uint_least64_t weekday_of_today{
                        static_cast<::std::uint_least64_t>(weekday_of_1st_day_this_year.u + day_of_the_year(tsp) - 2u) %
                            ndays +
                        1u};
                    ::std::uint_least64_t weeknum{static_cast<::std::uint_least64_t>(day_of_the_year(tsp) +
                                                                                     weekday_of_1st_day_this_year.u -
                                                                                     weekday_of_today - 1u) /
                                                  ndays};
            ...
            case char_literal_v<u8'V', char_type>: {
                ...
                else
                {
                    ...
                    auto weeknum{static_cast<::std::uint_least8_t>(
                        static_cast<::std::uint_least16_t>(day_of_the_year(tsp) - weekday(tsp) +
                                                           weekday_of_1st_day_this_year - 1u) /
                        7u)};
            ...
            case char_literal_v<u8'W', char_type>: {
                iter = chrono_two_digits_impl<false>(
                    iter, static_cast<::std::uint_least16_t>(day_of_the_year(tsp) - weekday(tsp) +
                                                             weekday(tsp.year, 1, 1) - 1u) /
                              7u);
                break;
            }
            case char_literal_v<u8'x', char_type>: {
...
template <::fast_io::manipulators::scalar_flags flags, ::std::integral char_type>
    requires(static_cast<::std::uint_least8_t>(static_cast<::std::uint_least8_t>(flags.time_flag) -
