    ...
    lc_locale &loc)
{
    ...
    else if (n == 0)
    {
        ...
        if constexpr (family == ::fast_io::win32_family::ansi_9x)
        {
            ...
            for (auto i{candidates}, ed{i + sz}; i != ed; ++i)
            {
                ::std::uint_least32_t env_size{::fast_io::win32::GetEnvironmentVariableA(
                    reinterpret_cast<char const *>(*i), reinterpret_cast<char *>(msys2_encoding),
                    msys2_encoding_size_restriction)};
                if (env_size < 2u)
                {
                    continue;
                }
        ...
        else
        {
            ...
            for (auto i{candidates}, ed{i + sz}; i != ed; ++i)
            {
                ::std::uint_least32_t env_size{
                    ::fast_io::win32::GetEnvironmentVariableW(*i, msys2_encoding, msys2_encoding_size_restriction)};
                if (env_size < 2u)
                {
                    continue;
                }
    ...
    constexpr ::std::size_t total_size{::fast_io::details::intrinsics::add_or_overflow_die_chain(
        sizeof(u8"fast_io_i18n.locale."), size_restriction, encoding_size_restriction, sizeof(u8".dll"))};
...
#endif
                = char16_t *;
            int ret{::fast_io::win32::GetUserDefaultLocaleName(reinterpret_cast<char16_may_alias_ptr>(it),
                                                               locale_name_max_len)};
            if (ret)
            {
                --ret;
...
template <::fast_io::win32_family family> class win32_family_l10n
{
  public:
    ...
    template <::fast_io::constructible_to_os_c_str path_type> explicit win32_family_l10n(path_type const &p)
    {
