...
concept lc_dynamic_reserve_printable = ::std::integral<char_type> && requires(T t, basic_lc_all<char_type> const *all,
																			  char_type *ptr, ::std::size_t size) {
...
template <::std::integral char_type, typename value_type>
	requires lc_dynamic_reserve_printable<char_type, ::std::remove_cvref_t<value_type>>
inline constexpr auto print_reserve_define(basic_lc_all<char_type> const *__restrict all, char_type *begin,
...
concept lc_printable =
	requires(basic_lc_all<char_type> const *all, ::fast_io::details::dummy_buffer_output_stream<char_type> out, T t) {
...
inline constexpr auto print_define_internal_shift(basic_lc_all<char_type> const *__restrict all,
												  parameter<value_type> para)
{
...
inline constexpr ::std::size_t calculate_lc_scatter_dynamic_reserve_size(basic_lc_all<char_type> const *__restrict all,
																		 T t, Args... args)
{
...
lc_scatter_print_with_dynamic_reserve_recursive(basic_lc_all<char_type> const *__restrict all,
												io_scatter_t *__restrict arr, char_type *__restrict ptr,
...
template <::std::integral char_type, typename T, typename... Args>
inline constexpr void
lc_scatter_print_with_dynamic_only_reserve_recursive(basic_lc_all<char_type> const *__restrict all,
...
[[msvc::forceinline]]
#endif
inline constexpr void lc_print_controls_line(basic_lc_all<typename output::char_type> const *__restrict lc, output out,
											 T t, Args... args)
{
	if constexpr (sizeof...(Args) == 0)
	{
...
template <::std::integral char_type, typename T, typename... Args>
inline constexpr void lc_scatter_print_recursive(basic_lc_all<char_type> const *__restrict lc, io_scatter_t *arr, T t,
												 Args... args)
{
	if constexpr (lc_scatter_printable<char_type, T>)
	{
...
inline constexpr void lc_print_fallback(basic_lc_all<typename output::char_type> const *__restrict lc, output out,
										Args... args)
{
	using char_type = typename output::char_type;
...
template <bool ln, output_stream output, typename... Args>
inline constexpr void
lc_print_status_define_further_decay(basic_lc_all<typename output::char_type> const *__restrict lc, output out,
									 Args... args)
{
	using char_type = typename output::char_type;
...
concept lc_print_status_define_okay_character_type =
