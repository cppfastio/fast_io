...
private:
	...
	{
		constexpr ::std::size_t block_size = sizeof(value_type) < 4096u ? 4096u : sizeof(value_type);
		...
		controller.controller_block.controller_start_ptr
			= static_cast<value_type**>(allocator::reallocate_n(controller.controller_block.controller_start_ptr, old_size * sizeof(value_type*), new_size * sizeof(value_type*)));
	...
	constexpr void init_grow() noexcept
	{
		...
		controller.controller_block.controller_after_ptr
			= controller.controller_block.controller_after_reserved_ptr
			= (controller.controller_block.controller_start_ptr + 1u);

		*controller.controller_block.controller_start_ptr
			= static_cast<value_type*>(allocator::allocate(block_size));
		...
		controller.back_block.curr_ptr
			= (controller.front_block.curr_ptr
				= (controller.front_block.begin_ptr
					...
					= *controller.controller_block.controller_start_ptr) + mid) + 1u;

		controller.front_block.end_ptr
			= controller.back_block.end_ptr
			= controller.front_block.begin_ptr + single_block_capacity;
	}
	...
	constexpr void grow_front() noexcept
	{
		constexpr ::std::size_t single_block_capacity{ sizeof(value_type) > 4096u ? 1u : 4096u / sizeof(value_type) };
		constexpr ::std::size_t block_size{ sizeof(value_type) > 4096u ? sizeof(value_type) : 4096u };
		constexpr ::std::size_t single_bloack_capacity = block_size / sizeof(value_type);
		constexpr ::std::size_t mid = single_bloack_capacity / 2u;
...
public:
	...
	constexpr ::std::size_t size() const noexcept
	{
		...
		return full_block_size * single_block_capacity
			+ (controller.back_block.curr_ptr - controller.back_block.begin_ptr - 1u)
			+ (controller.front_block.end_ptr - controller.front_block.curr_ptr);
	}
