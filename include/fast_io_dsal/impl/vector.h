...
inline constexpr ::std::size_t cal_grow_twice_size(::std::size_t cap) noexcept
{
    ...
    if(cap == mx_value) { ::fast_io::fast_terminate(); }
    else if(cap > mx_half_value)
    {
        if constexpr(trivial) { return mx_value; }
        else { return 1; }
    }
    else if(cap == 0) { return size; }
...
{
    if(end <= m->curr_ptr) { m->curr_ptr = end; }
    else
    {
...
template <typename allocator>
inline constexpr void check_size_and_assign_align(vector_model* m,
...
template <typename allocator>
inline constexpr void check_size_and_append_align(vector_model* m,
                                                  ::std::size_t alignment,
                                                  char8_t const* begin,
                                                  char8_t const* end) noexcept
{
...
template <typename allocator>
inline constexpr void check_size_and_construct_align(vector_model* m,
                                                     ::std::size_t alignment,
                                                     char8_t const* begin,
                                                     char8_t const* end) noexcept
{
...
public:
...
#endif
#endif
    ::std::conditional_t<alloc_with_status, empty, void*> allochdl{};

    [[nodiscard]] constexpr iterator begin() noexcept { return imp.begin_ptr; }

    [[nodiscard]] constexpr iterator end() noexcept { return imp.curr_ptr; }

    [[nodiscard]] constexpr const_iterator begin() const noexcept { return imp.begin_ptr; }

    [[nodiscard]] constexpr const_iterator end() const noexcept { return imp.curr_ptr; }

    [[nodiscard]] constexpr const_iterator cbegin() const noexcept { return imp.begin_ptr; }

    [[nodiscard]] constexpr const_iterator cend() const noexcept { return imp.curr_ptr; }

    [[nodiscard]] constexpr pointer data() noexcept { return imp.begin_ptr; }

    [[nodiscard]] constexpr const_pointer data() const noexcept { return imp.begin_ptr; }

    [[nodiscard]] constexpr bool is_empty() const noexcept { return imp.begin_ptr == imp.curr_ptr; }

    [[nodiscard]] constexpr bool empty() const noexcept { return imp.begin_ptr == imp.curr_ptr; }

    constexpr void clear() noexcept
    {
        if constexpr(!::std::is_trivially_destructible_v<value_type>)
        {
            for(auto old_i{imp.begin_ptr}, old_e{imp.curr_ptr}; old_i != old_e; ++old_i) { old_i->~value_type(); }
        }
    ...
    {
        ::fast_io::containers::details::vector_internal<T> tmp{this->imp};
        this->imp = other.imp;
        other.imp = tmp;
    }
...
private:
...
#endif
        {
            if constexpr(typed_allocator_type::has_deallocate) { typed_allocator_type::deallocate(imp.begin_ptr); }
            else
            {
    ...
    struct partial_destroy
    {
        ...
        constexpr ~partial_destroy()
        {
            ...
            for(; begin != thisvec->imp.curr_ptr; ++begin) { begin->~value_type(); }
...
public:
    ...
    constexpr vector(size_type n, value_type const& value) noexcept(::std::is_nothrow_copy_constructible_v<value_type>)
    {
        ...
        if constexpr(::std::is_nothrow_copy_constructible_v<value_type>)
        {
            ...
            for(auto p{begin_ptr}; p != e; ++p) { ::std::construct_at(p, value); }
        }
        else
        {
            ...
            for(; imp.curr_ptr != e; ++imp.curr_ptr) { ::std::construct_at(imp.curr_ptr, value); }
            des.thisvec = nullptr;
        }
    }
        ...
        ::std::is_nothrow_default_constructible_v<value_type>)
    {
        ...
        else
        {
            ...
            {
                auto e = imp.end_ptr = imp.curr_ptr = (imp.begin_ptr = begin_ptr) + n;
                for(auto p{begin_ptr}; p != e; ++p) { new (p) value_type(); }
            }
            else
            {
                imp.curr_ptr = imp.begin_ptr = begin_ptr;
                auto e = imp.end_ptr = imp.begin_ptr + n;
                run_destroy des(this);
                for(; imp.curr_ptr != e; ++imp.curr_ptr) { new (imp.curr_ptr) value_type(); }
                des.thisvec = nullptr;
            }
        }
    }
    ...
    constexpr vector(::std::from_range_t, R&& rg) noexcept(::std::is_nothrow_copy_constructible_v<value_type>)
    {
        ...
        else { this->append_uncounted_range_impl(::std::ranges::begin(rg), ::std::ranges::end(rg)); }
    }

    template <::fast_io::containers::details::container_compatible_range R>
    constexpr vector(void* alloc_handler, ::std::from_range_t, R&& rg) noexcept(vector(::std::from_range, rg))
        requires (alloc_with_status)
        : vector(::std::from_range, ::fast_io::freestanding::forward<R>(rg))
    {
    }
    ...
    constexpr vector(vector&& vec) noexcept : imp(vec.imp) { vec.imp = {}; }
...
private:
    ...
    __cpp_constexpr_dynamic_alloc >= 201907L
...
#endif
            {
                ...
                {
                    if(mxv < newcap) { ::fast_io::fast_terminate(); }
                }
    ...
    __cpp_constexpr_dynamic_alloc >= 201907L
        ...
        {
            if constexpr(typed_allocator_type::has_deallocate) { typed_allocator_type::deallocate(imp.begin_ptr); }
            else { typed_allocator_type::deallocate_n(imp.begin_ptr, cap); }
        }
        else { typed_allocator_type::deallocate_n(imp.begin_ptr, cap); }
    ...
    inline constexpr void grow_to_size_nearest_impl(size_type leastcap) noexcept
    {
        ...
        if(mx_value < cap) { cap = mx_value2; }
        else { cap <<= 1; }
        if(cap < leastcap) { cap = leastcap; }
        if constexpr(1 < sizeof(value_type))
        {
            if(mx_value2 < cap) { ::fast_io::fast_terminate(); }
        }
                                             ...
                                             InputIt last) noexcept(::std::is_nothrow_copy_constructible_v<value_type>)
    {
        ...
        for(; first != last; ++first, ++ptr) { ::std::construct_at(ptr, *first); }
        imp.curr_ptr = ptr;
    }
    ...
    inline constexpr void append_common_impl(InputIt first,
                                             InputIt last) noexcept(::std::is_nothrow_copy_constructible_v<value_type>)
    {
        for(; first != last; ++first, ++this->imp.curr_ptr) { ::std::construct_at(this->imp.curr_ptr, *first); }
    }

public:
    constexpr void reserve(size_type n) noexcept
    {
        if(n <= static_cast<::std::size_t>(imp.end_ptr - imp.begin_ptr)) { return; }
        constexpr bool asan_activated{::fast_io::details::asan_state::current ==
                                      ::fast_io::details::asan_state::activate};
        if constexpr(asan_activated) { grow_to_size_impl(n); }
        else { grow_to_size_nearest_impl(n); }
    }

    constexpr void assign(size_type n, value_type const& value) noexcept
        requires (::fast_io::freestanding::is_trivially_relocatable_v<value_type>)
    {
        if(n > static_cast<::std::size_t>(imp.end_ptr - imp.begin_ptr)) { grow_to_size_impl(n); }
        ...
        requires (!::fast_io::freestanding::is_trivially_relocatable_v<value_type>)
    {
        ...
        if(n > static_cast<::std::size_t>(imp.end_ptr - imp.begin_ptr)) { grow_to_size_impl(n); }
        run_destroy des{this};
        for(auto end{imp.begin_ptr + n}; imp.curr_ptr != end; ++imp.curr_ptr) { new (imp.curr_ptr) value_type(value); }
        des.thisvec = nullptr;
    }
        ...
        requires (::fast_io::freestanding::is_trivially_relocatable_v<value_type>)
    {
        ...
        {
            auto const size{static_cast<::std::size_t>(last - first)};
            if(size > static_cast<::std::size_t>(imp.end_ptr - imp.begin_ptr)) { grow_to_size_impl(size); }
            assign_common_impl(first, last);
        }
    }
        ...
        requires (!::fast_io::freestanding::is_trivially_relocatable_v<value_type>)
    {
        ...
        else { assign_common_impl(first, last); }
        des.thisvec = nullptr;
    }
        ...
        requires (::fast_io::freestanding::is_trivially_relocatable_v<value_type>)
    {
        if constexpr(::fast_io::freestanding::is_zero_default_constructible_v<value_type>)
        {
            ...
            if constexpr(1 < sizeof(value_type))
            {
                if(mxv < n) { ::fast_io::fast_terminate(); }
            }
            if constexpr(alignof(value_type) <= allocator_type::default_alignment)
            {
        ...
        {
            for(; imp.curr_ptr != new_e; ++imp.curr_ptr) { new (imp.curr_ptr) value_type(); }
            return;
        }
        // else (n > capacity())
        grow_to_size_impl(n);
        for(; imp.curr_ptr != imp.end_ptr; ++imp.curr_ptr) { new (imp.curr_ptr) value_type(); }
        return;
    }
        ...
        requires (::fast_io::freestanding::is_trivially_relocatable_v<value_type>)
    {
        ...
        for(; imp.curr_ptr != imp.end_ptr; ++imp.curr_ptr) { new (imp.curr_ptr) value_type(value); }
        return;
    }
        ...
        requires (!::fast_io::freestanding::is_trivially_relocatable_v<value_type>)
    {
        ...
        if(new_e <= imp.curr_ptr)
        {
            for(auto ptr{new_e}; ptr != imp.curr_ptr; ++ptr) { ptr->~value_type(); }
            imp.curr_ptr = new_e;
            return;
        }
        ...
        for(; imp.curr_ptr != new_e; ++imp.curr_ptr) { new (imp.curr_ptr) value_type(value); }
        des.begin = nullptr;
        des.thisvec = nullptr;
    }
...
private:
    ...
    __cpp_constexpr_dynamic_alloc >= 201907L
        ...
        else { new_end = imp.begin_ptr + new_cap; }
        {
    ...
    __cpp_constexpr_dynamic_alloc >= 201907L
            ...
            {
                if constexpr(typed_allocator_type::has_deallocate) { typed_allocator_type::deallocate(imp.begin_ptr); }
                else { typed_allocator_type::deallocate_n(imp.begin_ptr, old_cap); }
            }
            else { typed_allocator_type::deallocate_n(imp.begin_ptr, old_cap); }
        ...
        else { imp.curr_ptr = new_end; }
        return imp.begin_ptr + pos;
    }

    constexpr iterator reserve_blank_impl(iterator first,
                                          iterator last,
                                          size_type cnt) noexcept(::std::is_nothrow_move_constructible_v<value_type>)
    {
...
public:
    ...
    {
        if(cnt == 0) { return pos - imp.begin_ptr + imp.begin_ptr; }
        iterator itr;
        if(static_cast<size_type>(imp.end_ptr - imp.curr_ptr) < cnt)
        {
            itr = grow_to_size_and_reserve_blank_impl(pos - imp.begin_ptr, cnt);
        }
        else { itr = reserve_blank_impl(pos - imp.begin_ptr + imp.begin_ptr, imp.curr_ptr, cnt); }
        ...
        struct restore_t
        {
            ...
            {
                for(; new_i != new_e; ++new_i, ++old_i) { ::std::construct_at(old_i, ::std::move(*new_i)); }
            }
        } restore{};
    ...
    constexpr iterator insert_uncounted_range_impl(const_iterator pos,
                                                   Iter first,
                                                   Sentinel last) noexcept(noexcept(this->push_back(*first)))
    {
        if(first == last) { return pos - imp.begin_ptr + imp.begin_ptr; }
...
public:
    ...
    {
        iterator itr;
        if(imp.curr_ptr == imp.end_ptr) { itr = grow_to_size_and_reserve_blank_impl(pos - imp.begin_ptr, 1); }
        else
        {
            itr = ::fast_io::freestanding::copy_backward(pos - imp.begin_ptr + imp.begin_ptr,
                                                         imp.curr_ptr,
                                                         imp.curr_ptr + 1);
        }
    ...
    constexpr iterator
        insert(const_iterator pos, size_type count, T const& value) noexcept(noexcept(this->push_back(value)))
    {
        iterator itr;
    ...
    template <::std::input_iterator InputIt>
    constexpr iterator
        insert(const_iterator pos, InputIt first, InputIt last) noexcept(noexcept(this->push_back(*first)))
    {
        if constexpr(::std::forward_iterator<InputIt>)
        {
            return insert_counted_range_impl<false>(pos, first, ::std::distance(first, last));
        }
        else { return insert_uncounted_range_impl(pos, first, last); }
    }
                                    ...
                                    R&& rg) noexcept(noexcept(this->push_back(*::std::ranges::begin(rg))))
    {
        ...
        else { return insert_uncounted_range_impl(pos, ::std::ranges::begin(rg), ::std::ranges::end(rg)); }
    }
    ...
    __cpp_constexpr_dynamic_alloc >= 201907L
        ...
        // else
        mut_pos->~value_type();
        for(auto ptr{mut_pos + 1}; ptr != imp.curr_ptr; ++ptr) { *(ptr - 1) = ::std::move(*ptr); }
        --imp.curr_ptr;
        return mut_pos;
    }
    ...
    __cpp_constexpr_dynamic_alloc >= 201907L
        ...
        // else
        for(auto ptr{mut_first}; ptr != mut_last; ++ptr) { ptr->~value_type(); }
    ...
    {
        if(imp.curr_ptr == imp.end_ptr) { return; }
        grow_to_size_impl(static_cast<::std::size_t>(imp.curr_ptr - imp.begin_ptr));
    }

    constexpr void pop_back() noexcept { (--imp.curr_ptr)->~value_type(); }

    [[nodiscard]] constexpr const_reference operator[] (size_type pos) const noexcept { return imp.begin_ptr[pos]; }

    [[nodiscard]] constexpr reference operator[] (size_type pos) noexcept { return imp.begin_ptr[pos]; }

    [[nodiscard]] constexpr const_reference front() const noexcept { return *imp.begin_ptr; }

    [[nodiscard]] constexpr reference front() noexcept { return *imp.begin_ptr; }
...
private:
    ...
    {
        auto const sum{static_cast<size_type>(imp.curr_ptr - imp.begin_ptr) + cnt};
        auto const capcty{static_cast<size_type>(imp.end_ptr - imp.begin_ptr)};
        if(capcty < sum)
        {
            if constexpr(SIZE_MAX < ::std::numeric_limits<size_type>::max())
            {
                if(SIZE_MAX < sum) { ::fast_io::fast_terminate(); }
            }
            this->grow_to_size_nearest_impl(sum);
        }
        for(size_type i{}; i != cnt; ++i, ++first, ++this->imp.curr_ptr)
        {
    ...
    {
        for(; first != last; ++first) { this->emplace_back(*first); }
    }
...
public:
    ...
    [[nodiscard]] constexpr const_reference back() const noexcept { return imp.curr_ptr[-1]; }

    [[nodiscard]] constexpr reference back() noexcept { return imp.curr_ptr[-1]; }
...
constexpr auto operator<=> (vector<T, allocator>& lhs, vector<T, allocator>& rhs) noexcept
{
    ...
    {
        if(auto result = *first1 <=> *first2; result != 0) { return result; }
    }
    if(first1 == last1) { return ordering_category_t(first2 <=> last2); }
