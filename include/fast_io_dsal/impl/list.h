...
class list_iterator
{
	...
	constexpr operator list_iterator<T,true>() requires(!isconst)
	{
		return {this->iter};
	}
};
...
public:
	...
	constexpr list() noexcept:imp{__builtin_addressof(imp),__builtin_addressof(imp)}
	{
	}
	...
	{
		constexpr
			size_type mxvl{SIZE_MAX/sizeof(node_type)};
		return mxvl;
	}
...
private:
	...
	constexpr auto allocate_construct_new_node(Args &&...args) noexcept(noexcept(value_type(::std::forward<Args>(args)...)))
	{
...
public:
	...
	constexpr list(list&& other) noexcept:imp(other.imp)
	{
