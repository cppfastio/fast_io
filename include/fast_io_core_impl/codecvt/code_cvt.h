...
template <::std::integral from_char_type, ::std::integral to_char_type, encoding_scheme from_scheme,
		  encoding_scheme to_scheme>
inline constexpr ::std::size_t
deco_reserve_size(io_reserve_type_t<to_char_type, basic_code_converter<from_char_type, from_scheme, to_scheme>>,
...
template <encoding_scheme from_scheme, encoding_scheme to_scheme, ::std::contiguous_iterator from_iter,
		  ::std::contiguous_iterator to_iter>
inline constexpr to_iter
deco_reserve_define(io_reserve_type_t<::std::iter_value_t<to_iter>,
									  basic_code_converter<::std::iter_value_t<from_iter>, from_scheme, to_scheme>>,
					basic_code_converter<::std::iter_value_t<from_iter>, from_scheme, to_scheme> &conv,
					from_iter src_first, from_iter src_last, to_iter dst) noexcept
{
	...
	{
		return details::codecvt::general_code_cvt<from_scheme, to_scheme>(conv.state, ::std::to_address(src_first),
																		  ::std::to_address(src_last), dst);
	}
	else
	{
		return dst +
			   (details::codecvt::general_code_cvt<from_scheme, to_scheme>(
					conv.state, ::std::to_address(src_first), ::std::to_address(src_last), ::std::to_address(dst)) -
				::std::to_address(dst));
	}
}
