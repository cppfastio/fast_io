...
template <typename T> struct tuint
{
    T quotient, remainder;
};

template <typename T> struct tuints
{
...
template <typename T> inline constexpr tuint<T> udivbigbysmalltosmalldefault(T u1, T u0, T v) noexcept
{
...
template <typename T> inline constexpr T shiftleft(T low, T high, unsigned shift) noexcept
{
...
template <typename T> inline constexpr T shiftright(T low, T high, unsigned shift) noexcept
{
#if defined(_MSC_VER) && !defined(__clang__) && defined(_M_AMD64)
    if constexpr (sizeof(T) == sizeof(long long unsigned))
    {
