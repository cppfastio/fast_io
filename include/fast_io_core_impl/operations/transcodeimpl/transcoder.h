...
{
	return basic_transcoder_t<typename decltype(print_alias_define(sct))::value_type, T *>{print_alias_define(::std::forward<stryp>(sct)), __builtin_addressof(t)};
}

template <::std::integral to_char_type, ::std::integral from_char_type, typename T>
inline constexpr auto status_io_print_forward(::fast_io::io_alias_type_t<to_char_type>, basic_transcoder_t<from_char_type, T *> t)
{
	return basic_transcoder_t<from_char_type, decltype(transcode_forward_ref<from_char_type, to_char_type>(*t.transcode))>{t.reference, transcode_forward_ref<from_char_type, to_char_type>(*t.transcode)};
}
...
concept transcode_imaginary_printable = ::std::integral<char_type> && requires() {
	...
	requires(::std::same_as<T, basic_transcoder_t<typename T::char_type, typename T::transcoder_value_type>> &&
