...
template <typename instmtype>
inline constexpr void pread_until_eof_bytes_cold_impl(instmtype insm, ::std::byte *first, ::std::byte *last,
...
#endif
inline constexpr
	typename instmtype::input_char_type *read_until_eof_cold_impl(instmtype insm,
	...
	else if constexpr (::fast_io::operations::decay::defines::has_scatter_read_some_underflow_define<instmtype>)
	{
		if constexpr (::fast_io::operations::decay::defines::has_ibuffer_basic_operations<instmtype>)
		{
			for (;;)
			{
				...
				basic_io_scatter_t<char_type> sc{first, len};
				auto [position,
					  position_io_scatter]{scatter_read_some_underflow_define(insm, __builtin_addressof(sc), 1)};
				if (position == 1)
				{
					return last;
				}
...
#endif
			= char_type *;
		return reinterpret_cast<char_type_ptr>(read_until_eof_bytes_cold_impl(
			insm, reinterpret_cast<::std::byte *>(first), reinterpret_cast<::std::byte *>(last)));
	}
...
inline constexpr ::std::byte *read_until_eof_bytes_cold_impl(instmtype insm, ::std::byte *first, ::std::byte *last)
{
						   ...
						   instmtype>)
	{
		...
		basic_io_scatter_t<char_type> sc{first, len};
		auto [position,
			  position_in_scatter]{scatter_read_until_eof_bytes_underflow_define(insm, __builtin_addressof(sc), 1)};
		if (position)
		{
			return last;
		}
...
template <typename instmtype>
inline constexpr typename instmtype::input_char_type *read_until_eof_decay(instmtype insm,
																		   typename instmtype::input_char_type *first,
																		   typename instmtype::input_char_type *last)
{
	using char_type = typename instmtype::input_char_type;
	if constexpr (::fast_io::operations::decay::defines::has_input_or_io_stream_mutex_ref_define<instmtype>)
	{
		...
		return ::fast_io::operations::decay::read_until_eof_decay(
...
inline constexpr ::std::byte *read_until_eof_bytes_decay(instmtype insm, ::std::byte *first, ::std::byte *last)
{
	...
	if constexpr (::fast_io::operations::decay::defines::has_input_or_io_stream_mutex_ref_define<instmtype>)
	{
		::fast_io::operations::decay::stream_ref_decay_lock_guard lg{
			::fast_io::operations::decay::input_stream_mutex_ref_decay(insm)};
		return ::fast_io::operations::decay::read_until_eof_bytes_decay(
			::fast_io::operations::decay::input_stream_unlocked_ref_decay(insm), first, last);
	}
	else
	{
