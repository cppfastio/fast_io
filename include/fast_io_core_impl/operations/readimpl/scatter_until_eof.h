...
scatter_read_until_eof_decay(instmtype insm, basic_io_scatter_t<typename instmtype::input_char_type> const *pscatters,
                             ::std::size_t n);

template <typename instmtype>
inline constexpr void scatter_read_until_eof_bytes_decay(instmtype insm, io_scatter_t const *pscatters,
                                                         ::std::size_t n);
...
{
template <typename instmtype>
inline constexpr io_scatter_status_t scatter_read_until_eof_cold_impl(
    instmtype insm, basic_io_scatter_t<typename instmtype::input_char_type> const *pscatters, ::std::size_t n);

template <typename instmtype>
inline constexpr io_scatter_status_t
scatter_pread_until_eof_cold_impl(instmtype insm,
...
#endif
inline constexpr io_scatter_status_t
scatter_read_until_eof_bytes_cold_impl(instmtype insm, io_scatter_t const *pscatters, ::std::size_t n)
{
                       ...
                       ::fast_io::operations::decay::defines::has_read_until_eof_bytes_underflow_define<instmtype>)
    {
        for (auto i{pscatters}, e{pscatters + n}; i != e; ++i)
        {
            ...
            if (baseit != baseed)
            {
                return {static_cast<::std::size_t>(i - pscatters), static_cast<::std::size_t>(baseit - base)};
            }
        }
        return {n, 0};
    }
                       ...
                       ::fast_io::operations::decay::defines::has_any_of_pread_bytes_operations<instmtype>)
    {
        ...
        ::fast_io::operations::decay::input_stream_seek_bytes_decay(
            insm, ::fast_io::fposoffadd_scatters(0, pscatters, status), ::fast_io::seekdir::cur);
        return status;
    }
    else if constexpr (sizeof(char_type) == 1 &&
    ...
    instmtype insm, basic_io_scatter_t<typename instmtype::input_char_type> const *pscatters, ::std::size_t n)
{
...
#endif
                = io_scatter_t *;
            return ::fast_io::details::scatter_read_until_eof_bytes_cold_impl(
                insm, reinterpret_cast<scattermayalias_ptr>(pscatters), n);
        }
        else
        {
                       ...
                       (::fast_io::operations::decay::defines::has_any_of_pread_bytes_operations<instmtype>))
    {
        auto status{::fast_io::details::scatter_pread_until_eof_cold_impl(insm, pscatters, n, 0)};
        ::fast_io::operations::decay::input_stream_seek_bytes_decay(
            insm, ::fast_io::details::scatter_fpos_mul<char_type>(::fast_io::fposoffadd_scatters(0, pscatters, status)),
            ::fast_io::seekdir::cur);
        return status;
    }
}
                             ...
                             ::std::size_t n)
{
    if constexpr (::fast_io::operations::decay::defines::has_input_or_io_stream_mutex_ref_define<instmtype>)
    {
        ...
        return ::fast_io::operations::decay::scatter_read_until_eof_decay(
...
template <typename instmtype>
inline constexpr io_scatter_status_t scatter_read_until_eof_bytes_decay(instmtype insm, io_scatter_t const *pscatters,
                                                                        ::std::size_t n)
{
    if constexpr (::fast_io::operations::decay::defines::has_input_or_io_stream_mutex_ref_define<instmtype>)
    {
    ...
    else if constexpr (::fast_io::operations::decay::defines::has_ibuffer_basic_operations<instmtype>)
    {
        ...
        {
            auto ret{
                ::fast_io::details::scatter_read_until_eof_bytes_cold_impl(insm, i, static_cast<::std::size_t>(e - i))};
            ret.position += static_cast<::std::size_t>(i - pscatters);
            return ret;
        }
        return {n, 0};
