...
[[__gnu__::__cold__]]
#endif
inline constexpr void print_control_dynamic_reserve_bad_path(output outstm,value_type t,std::size_t sizep1)
{
...
inline constexpr void print_control(output outstm,T t)
{
	...
	else if constexpr(dynamic_reserve_printable<char_type,value_type>)
	{
		...
		else
		{
			...
			print_control_dynamic_reserve_bad_path<line,value_type>(outstm,t,size);
		}
	}
...
[[msvc::forceinline]]
#endif
inline constexpr void print_controls_line_multi_impl(output outstm,T t,Args ...args)
{
	if constexpr(n==0)
	{
		using char_type = typename output::char_type;
				...
				static_assert(reserve_paras_info.total_size<static_cast<std::size_t>(PTRDIFF_MAX),"reserve size cannot be zero");
				{
					...
					if constexpr(buffer_output_stream<output>)
					{
						...
						if constexpr(constant_buffer_output_stream_require_size_impl<output,buffer_size>)
						{
							...
							obuffer_set_curr(outstm,curr_ptr);
						}
						else
						{
							...
							write(outstm,start,ptr);
						}
					}
				}
				if constexpr(reserve_paras_info.n!=sizeof...(Args))
					print_controls_line_multi_impl<line,reserve_paras_info.n>(outstm,args...);
			}
			else
			{
				if constexpr(sizeof...(Args)==0)
					print_control<line>(outstm,t);
				else
				{
					print_control<false>(outstm,t);
					print_controls_line_multi_impl<line,0>(outstm,args...);
				}
			}
		}
		else
	...
	{
		print_controls_line_multi_impl<line,n-1>(outstm,args...);
	}
}
...
inline constexpr void print_fallback(output outstm,Args ...args)
{
	using char_type = typename output::char_type;
	if constexpr(line&&sizeof...(Args)==0)
	{
		write(outstm,__builtin_addressof(char_literal_v<u8'\n',char_type>),__builtin_addressof(char_literal_v<u8'\n',char_type>)+1);
	}
	else
	{
		...
		else
		{
				...
				((scatter_printable<typename output::char_type,Args>||reserve_printable<typename output::char_type,Args>||dynamic_reserve_printable<typename output::char_type,Args>)&&...))
			{
				...
				if constexpr((scatter_printable<typename output::char_type,Args>&&...))
				{
					...
					{
						scatter_constant_write<n>(outstm,scatters);
					}
					else
					{
						scatter_write(outstm,{scatters,n});
					}
				}
...
template<typename output,typename ...Args>
concept print_freestanding_decay_okay = std::is_trivially_copyable_v<output>&&((status_output_stream<output>&&requires(output outstm,Args ...args)
{
	print_status_define<false>(outstm,args...);
	print_status_define<true>(outstm,args...);
...
template<typename output,typename ...Args>
concept print_freestanding_okay = requires(output&& outstm,Args&& ...args)
{
	::fast_io::details::print_freestanding_deacy_okay_dummy(::fast_io::manipulators::output_stream_ref(outstm),io_print_forward<typename std::remove_cvref_t<output>::char_type>(io_print_alias(args))...);
};

template<bool line,output_stream output,typename ...Args>
requires print_freestanding_decay_okay_no_status<output,Args...>
inline constexpr void print_freestanding_decay_no_status(output outstm,Args ...args)
{
	...
	{
		if constexpr(sizeof...(Args)==0&&line)
			put(outstm,char_literal_v<u8'\n',char_type>);
		else
		{
#if defined(__OPTIMIZE__) || defined(__OPTIMIZE_SIZE__)
			::fast_io::details::decay::print_controls_line_multi_impl<line,0>(outstm,args...);
#else
			if constexpr(line)
			{
				::fast_io::details::decay::print_controls_line<line>(outstm,args...);
			}
			else
			{
				(::fast_io::details::decay::print_control<false>(outstm,args),...);
			}
#endif
		}
	}
	...
	{
		::fast_io::details::decay::print_fallback<line>(outstm,args...);
	}
}

template<bool line,typename output,typename ...Args>
requires print_freestanding_decay_okay<output,Args...>
inline constexpr void print_freestanding_decay(output outstm,Args ...args)
{
	if constexpr(status_output_stream<output>)
		print_status_define<line>(outstm,args...);
	else
		print_freestanding_decay_no_status<line>(outstm,args...);
}
...
[[__gnu__::__cold__]]
#endif
inline constexpr void print_freestanding_decay_cold(output outstm,Args ...args)
{
	if constexpr(status_output_stream<output>)
		print_status_define<line>(outstm,args...);
	else
