...
template <typename T>
concept has_status_scan_define = requires(T optstm) { status_scan_define<true>(optstm, 0); };
...
template <typename T>
concept has_read_until_eof_bytes_underflow_define =
	requires(T instm, ::std::byte *ptr) { read_until_eof_bytes_underflow_define(instm, ptr, ptr); };

template <typename T>
concept has_scatter_read_some_bytes_underflow_define =
	requires(T instm, ::fast_io::io_scatter_t *scatter, ::std::size_t len) {
...
template <typename T>
concept has_scatter_read_until_eof_bytes_underflow_define =
	requires(T instm, ::fast_io::io_scatter_t *scatter, ::std::size_t len) {
...
concept has_scatter_read_some_underflow_define =
	requires(T instm, ::fast_io::basic_io_scatter_t<typename decltype(instm)::input_char_type> const *pscatter,
			 ::std::size_t len) { scatter_read_some_underflow_define(instm, pscatter, len); };

template <typename T>
concept has_scatter_read_all_underflow_define =
	requires(T instm, ::fast_io::basic_io_scatter_t<typename decltype(instm)::input_char_type> const *pscatter,
			 ::std::size_t len) { scatter_read_all_underflow_define(instm, pscatter, len); };

template <typename T>
concept has_scatter_read_until_eof_underflow_define =
	requires(T instm, ::fast_io::basic_io_scatter_t<typename decltype(instm)::input_char_type> const *pscatter,
			 ::std::size_t len) { scatter_read_until_eof_underflow_define(instm, pscatter, len); };

template <typename T>
concept has_ibuffer_underflow_never_define =
	requires(T instm) { ibuffer_underflow_never_define(::fast_io::operations::output_stream_ref(instm)); };
...
template <typename T>
concept has_pread_until_eof_bytes_underflow_define =
	requires(T instm, ::std::byte *ptr) { pread_until_eof_bytes_underflow_define(instm, ptr, ptr, 0); };
...
template <typename T>
concept has_scatter_pread_until_eof_bytes_underflow_define =
	requires(T instm, ::fast_io::io_scatter_t *scatter, ::std::size_t len) {
...
concept has_scatter_pread_some_underflow_define =
	requires(T instm, ::fast_io::basic_io_scatter_t<typename decltype(instm)::input_char_type> const *scatter,
			 ::std::size_t len) { scatter_pread_some_underflow_define(instm, scatter, len, 0); };

template <typename T>
concept has_scatter_pread_all_underflow_define =
	requires(T instm, ::fast_io::basic_io_scatter_t<typename decltype(instm)::input_char_type> const *scatter,
			 ::std::size_t len) { scatter_pread_all_underflow_define(instm, scatter, len, 0); };

template <typename T>
concept has_scatter_pread_until_eof_underflow_define =
	requires(T instm, ::fast_io::basic_io_scatter_t<typename decltype(instm)::input_char_type> const *scatter,
			 ::std::size_t len) { scatter_pread_until_eof_underflow_define(instm, scatter, len, 0); };
...
concept readable =
	...
	(sizeof(typename stmtype::output_char_type) == 1 && has_any_of_read_or_seek_pread_bytes_operations<stmtype>);

template <typename stmtype>
concept bytes_readable =
	has_any_of_read_or_seek_pread_bytes_operations<stmtype> ||
	(sizeof(typename stmtype::output_char_type) == 1 && has_any_of_read_or_seek_pread_operations<stmtype>);

template <typename stmtype>
concept preadable =
	has_any_of_pread_or_seek_read_operations<stmtype> ||
	(sizeof(typename stmtype::output_char_type) == 1 && has_any_of_pread_or_seek_read_bytes_operations<stmtype>);

template <typename stmtype>
concept bytes_preadable =
	has_any_of_pread_or_seek_read_bytes_operations<stmtype> ||
	(sizeof(typename stmtype::output_char_type) == 1 && has_any_of_pread_or_seek_read_operations<stmtype>);
