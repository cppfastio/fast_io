...
concept has_output_stream_transcode_deco_filter_define = requires(T t,P p)
{
	output_stream_transcode_deco_filter_define(t,p);
};

template<typename T,typename P>
concept has_input_stream_transcode_deco_filter_define = requires(T t,P p)
{
	input_stream_transcode_deco_filter_define(t,p);
};

template<typename T,typename P>
concept has_io_stream_transcode_deco_filter_define = requires(T t,P p)
{
	io_stream_transcode_deco_filter_define(t,p);
};
...
inline constexpr auto transcode_input_decos_decay(T t,D &&deco,Args&& ...args)
{
	if constexpr(::fast_io::operations::decay::defines::has_input_stream_transcode_deco_filter_define<T,D>)
	{
		...
		else
		{
			...
			::fast_io::operations::decay::add_input_decos_decay(::fast_io::operations::input_stream_deco_filter_ref(newdecof),::fast_io::freestanding::forward<Args>(args)...);
			return newdecof;
		}
	}
	else
...
inline constexpr auto transcode_output_decos_decay(T t,D &&deco,Args&& ...args)
{
	if constexpr(::fast_io::operations::decay::defines::has_output_stream_transcode_deco_filter_define<T,D>)
	{
		...
		else
		{
			...
			::fast_io::operations::decay::add_output_decos_decay(::fast_io::operations::output_stream_deco_filter_ref(newdecof),::fast_io::freestanding::forward<Args>(args)...);
			return newdecof;
		}
	}
	else
...
inline constexpr auto transcode_io_decos_decay(T t,D &&deco,Args&& ...args)
{
	...
	{
		auto newdecof{io_stream_transcode_deco_filter_define(t,::fast_io::freestanding::forward<D>(deco))};
		::fast_io::operations::decay::add_io_decos_decay(::fast_io::operations::io_stream_deco_filter_ref(newdecof),::fast_io::freestanding::forward<Args>(args)...);
		return newdecof;
	}
}
...
template<typename T>
concept has_input_or_io_stream_transcode_deco_filter_ref_define = has_input_stream_transcode_deco_filter_ref_define<T>||
					has_io_stream_transcode_deco_filter_ref_define<T>;

template<typename T>
concept has_output_or_io_stream_transcode_deco_filter_ref_define = has_output_stream_transcode_deco_filter_ref_define<T>||
					has_io_stream_transcode_deco_filter_ref_define<T>;
