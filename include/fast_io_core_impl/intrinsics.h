...
template <typename U> struct ul_generic_x2_little_endian
{
    U low, high;
};
template <typename U> struct ul_generic_x2_big_endian
{
    U high, low;
};
...
template <typename T> inline constexpr ::std::size_t cal_allocation_size_or_die(::std::size_t size) noexcept
{
#if defined(_MSC_VER) && !defined(__clang__)
...
template <typename U> inline constexpr U shiftright_naive(U low_part, U high_part, ::std::uint_least8_t shift) noexcept
{
...
template <typename U> inline constexpr U shiftright(U low_part, U high_part, ::std::uint_least8_t shift) noexcept
{
#if __cpp_lib_is_constant_evaluated >= 201811L
    if (::std::is_constant_evaluated())
