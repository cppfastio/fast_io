...
template <typename T> inline constexpr decltype(auto) io_print_alias(T &&t) noexcept
{
...
template <::std::integral char_type, typename T> inline constexpr auto io_print_forward(T &&t) noexcept
{
    using no_cvref_t = ::std::remove_cvref_t<T>;
    if constexpr (::std::is_function_v<no_cvref_t>)
    {
...
template <::std::integral char_type, typename T> inline constexpr auto io_scan_forward(T t) noexcept
{
...
template <typename T> inline constexpr auto io_scan_alias(T &&t) noexcept
{
