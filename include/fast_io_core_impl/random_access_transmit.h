...
inline constexpr ::std::uintmax_t zero_copy_random_access_transmit_impl(output &outp, input &inp,
																		::std::intmax_t offset)
{
...
template <output_stream output, input_stream input>
inline constexpr ::std::uintmax_t zero_copy_random_access_transmit_impl(output &outp, input &inp,
																		::std::intmax_t offset, ::std::uintmax_t sz)
{
...
inline constexpr ::std::uintmax_t random_access_transmit(output &&outp, offset_type offset, input &&in)
{
	...
	print_freestanding(
		::std::forward<output>(outp),
		manip::random_access_transmission<input, offset_type, ::std::uintmax_t>(transmitted, offset, in));
	return transmitted;
}
...
inline constexpr sz_type random_access_transmit(output &&outp, offset_type offset, input &&in, sz_type bytes)
{
	...
	print_freestanding(::std::forward<output>(outp),
					   manip::random_access_transmission_with_size<input, offset_type, ::std::uintmax_t>(
						   transmitted, offset, in, bytes));
	return transmitted;
}
