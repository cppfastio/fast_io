...
template <::std::input_iterator fwd_iter, typename T> inline constexpr void fill(fwd_iter first, fwd_iter last, T value)
{
    for (; first != last; ++first)
        *first = value;
}
...
template <::std::input_iterator Iter, ::std::input_iterator Iter2> struct mismatch_result
{
    Iter first;
    Iter2 second;
};
