...
template <::std::integral char_type, typename T> struct io_strlike_type_t
{
    explicit constexpr io_strlike_type_t() noexcept = default;
};

template <::std::integral char_type, typename T> inline constexpr io_strlike_type_t<char_type, T> io_strlike_type{};
...
template <typename char_type, typename T>
concept strlike =
    ::std::integral<char_type> && ::std::is_default_constructible_v<T> && (requires(char_type const *first) {
