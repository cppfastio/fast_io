...
template <typename T> FAST_IO_MODULE_EXPORT struct basic_io_scatter_t
{
    using value_type = T;
    T const *base;
    ::std::size_t len;
...
template <typename T> FAST_IO_MODULE_EXPORT struct basic_message_hdr
{
...
template <typename T> FAST_IO_MODULE_EXPORT struct io_type_t
{
    using type = T;
};
template <typename T> FAST_IO_MODULE_EXPORT inline constexpr io_type_t<T> io_type{};

template <::std::integral char_type> FAST_IO_MODULE_EXPORT struct cross_code_cvt_t
{
    using value_type = char_type;
    basic_io_scatter_t<value_type> scatter;
};

template <::std::integral char_type, typename T> FAST_IO_MODULE_EXPORT struct io_reserve_type_t
{
...
template <::std::integral char_type> FAST_IO_MODULE_EXPORT struct basic_reserve_scatters_define_result
{
...
template <::std::integral char_type> FAST_IO_MODULE_EXPORT struct io_alias_type_t
{
    explicit constexpr io_alias_type_t() noexcept = default;
};

template <::std::integral char_type> FAST_IO_MODULE_EXPORT inline constexpr io_alias_type_t<char_type> io_alias_type{};

template <::std::integral char_type> FAST_IO_MODULE_EXPORT struct try_get_result
{
    char_type ch;
    bool eof;
};

template <typename in_char_type, typename out_char_type> FAST_IO_MODULE_EXPORT struct deco_result
{
...
template <typename T> inline constexpr io_cookie_type_t<T> io_cookie_type{};
