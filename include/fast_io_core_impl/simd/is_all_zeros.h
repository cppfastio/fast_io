...
template <unsigned pos, typename T> inline constexpr bool is_all_zeros_recursive_impl(T const &v2) noexcept
{
...
template <typename T, ::std::size_t N> inline constexpr bool is_all_zeros(simd_vector<T, N> const &vec) noexcept
{
