...
template <typename T, typename Func> inline constexpr void generic_simd_self_op_impl(T &selfval, Func func) noexcept
{
    constexpr ::std::size_t N{::std::remove_cvref_t<T>::size()};
    for (::std::size_t i{}; i != N; ++i)
    {
...
template <typename T> inline constexpr auto create_value_mx() noexcept
{
