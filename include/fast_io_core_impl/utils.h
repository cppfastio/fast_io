...
template <typename R, typename... Args> struct make_noexcept
{
};

template <typename R, typename... Args> struct make_noexcept<R(Args...)>
{
    using type = R(Args...) noexcept;
};

template <typename R, typename... Args> struct make_noexcept<R(Args...) noexcept>
{
    using type = R(Args...) noexcept;
};

template <typename R, typename... Args> using make_noexcept_t = typename make_noexcept<R, Args...>::type;
...
template <typename mutx_type> struct io_lock_guard
{
...
template <typename stream_type> struct io_flush_guard
{
...
template <::std::size_t v> inline constexpr __uint128_t uint128_t_bsv{calculate_byteswap_ff(v)};
#endif
...
template <details::my_unsigned_integral U> inline constexpr U byte_swap(U a) noexcept
{
...
template <details::my_unsigned_integral U> inline constexpr U big_endian(U u) noexcept
{
...
template <details::my_unsigned_integral U> inline constexpr U little_endian(U u) noexcept
{
...
template <my_integral T> inline constexpr T compile_time_pow(T base, ::std::size_t pow) noexcept
{
    T t = 1;
...
template <my_integral T> inline constexpr my_make_unsigned_t<T> cal_int_max() noexcept
{
...
template <my_integral T> inline constexpr T get_int_max() noexcept
{
    constexpr T v{static_cast<T>(cal_int_max<T>())};
    return v;
}
template <my_integral T> inline constexpr auto get_int_max_unsigned() noexcept
{
...
template <my_integral T, char8_t base = 10> inline constexpr ::std::size_t cal_max_int_size() noexcept
{
    ::std::size_t i{};
...
template <::std::integral char_type, bool iobuf = false> inline constexpr ::std::size_t cal_buffer_size()
{
