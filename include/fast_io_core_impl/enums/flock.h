...
template <::std::signed_integral T> struct basic_flock_request
{
    using int_type = T;
...
{
    template <typename RequestType> constexpr void lock(RequestType const &) noexcept {}
    template <typename RequestType> constexpr void unlock(RequestType const &) noexcept {}
    template <typename RequestType> constexpr bool try_lock(RequestType const &) noexcept
    {
        return true;
    }
};
