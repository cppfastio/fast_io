...
template <ip_flags flags, typename T> struct ip_manip_t
{
    using value_type = T;
...
template <ip_scan_flags flags, typename T> struct ip_scan_manip_t
{
    using value_type = T;
    using manip_tag = manip_tag_t;
#ifndef __INTELLISENSE__
...
template <scalar_flags flags, typename T> struct scalar_manip_t
{
    using value_type = T;
...
template <typename T> struct whole_get_t
{
    using value_type = T;
    using manip_tag = manip_tag_t;
...
template <typename inttype> struct unsigned_integer_alias_type_traits_helper
{
...
template <typename inttype> struct signed_integer_alias_type_traits_helper
{
    using alias_type = ::std::conditional_t<
...
template <typename inttype> struct integer_alias_type_traits
{
    using alias_type = ::std::conditional_t<
...
template <typename inttype> struct integer_full_alias_type_traits
{
...
template <typename flt> struct float_alias_type_traits
{
    using alias_type = flt;
};

#ifdef __STDCPP_FLOAT64_T__
template <> struct float_alias_type_traits<double>
{
    using alias_type =
...
template <> struct float_alias_type_traits<float>
{
    using alias_type =
...
template <> struct float_alias_type_traits<long double>
{
...
template <> struct float_alias_type_traits<__float16>
{
...
template <scalar_flags flags, typename T> struct scalar_manip_precision_t
{
    using scalar_flags_type = scalar_flags;
    using manip_tag = manip_tag_t;
#ifndef __INTELLISENSE__
...
template <scalar_placement flags, typename T> struct width_t
{
    using manip_tag = manip_tag_t;
#ifndef __INTELLISENSE__
#if __has_cpp_attribute(msvc::no_unique_address)
...
template <scalar_placement flags, typename T, ::std::integral ch_type> struct width_ch_t
{
    using manip_tag = manip_tag_t;
...
template <typename T> struct width_runtime_t
{
    using manip_tag = manip_tag_t;
...
template <typename T, ::std::integral ch_type> struct width_runtime_ch_t
{
    using manip_tag = manip_tag_t;
    using char_type = ch_type;
    scalar_placement placement;
...
{
    return ::fast_io::details::scalar_flags_int_cache<
        ::fast_io::details::base_mani_flags_cache<16, true, shbase, true>>(t);
}
...
template <::std::integral inttype> inline constexpr ::std::remove_cvref_t<inttype> bitfieldvw(inttype v) noexcept
{
    return v;
}
...
template <::std::integral char_type, ::std::size_t base> inline constexpr auto generate_base_prefix_array() noexcept
{
...
template <::std::size_t base> inline constexpr ::std::size_t print_showbase_length{compute_print_showbase_length(base)};
