...
{
template <typename T> struct chvw_t
{
    using manip_tag = manip_tag_t;
    T reference;
};

template <::std::integral ch_type, ::std::size_t N> struct small_scatter_t
{
    using manip_tag = manip_tag_t;
    ch_type const *base{};
    ::std::size_t len{};
...
template <::std::integral T> inline constexpr chvw_t<T> chvw(T ch) noexcept
{
    return {ch};
}
...
template <::std::integral T> inline constexpr void os_c_str(decltype(nullptr), ::std::size_t) noexcept = delete;
...
template <typename T, ::std::size_t N> struct my_constant_passer
{
    using value_type = T;
