...
template <bool char_execharset> inline ::std::uint_least32_t detect_length(char unsigned const *buffer) noexcept
{
...
template <typename char_type> struct ch_get_t
{
    using manip_tag = manip_tag_t;
    char_type &reference;
};

template <::fast_io::details::my_integral T> inline constexpr ch_get_t<T &> ch_get(T &reference) noexcept
{
    return {reference};
}
