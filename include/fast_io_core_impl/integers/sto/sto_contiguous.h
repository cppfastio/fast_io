...
scan_int_contiguous_none_space_part_define_impl(char_type const *first, char_type const *last, T &t) noexcept
{
	...
	if constexpr (base == 10 && sizeof(char_type) == 1 && sizeof(unsigned_type) <= sizeof(::std::uint_least64_t))
	{
		...
		else [[unlikely]]
		{
...
#endif
			{
				auto [it2, ec] = scan_int_contiguous_none_simd_space_part_define_naive_impl<base>(first, last, res);
				if (ec != parse_code::ok)
				{
					return {it2, ec};
				}
			...
			{
				auto [it2, ec] = scan_int_contiguous_none_simd_space_part_define_none_constexpr_impl<base>(first, last, res);
				if (ec != parse_code::ok)
				{
					return {it2, ec};
				}
