...
inline constexpr char_type *prtrsv_inaddr_common_define_impl(char_type *it,
                                                             char unsigned const *__restrict start) noexcept
{
    ...
    else if constexpr (shorten)
    {
        ...
        for (auto i{start}; i != e;)
        {
            ...
            else
            {
                ...
                it = print_reserve_integral_define<16, false, false, false, uppercase, full>(it,
                                                                                             ::fast_io::big_endian(*i));
                ++i;
            }
        }
    }
...
template <::std::integral char_type> inline constexpr char_type *prtrsv_ipport_zero_define_impl(char_type *it) noexcept
{
    if constexpr (::std::same_as<char_type, char>)
    {
...
concept inaddrnocvrefimpl = ::std::same_as<T, ::fast_io::posix_in_addr> ||
                            ::std::same_as<T, ::fast_io::posix_in6_addr> || ::std::same_as<T, ::fast_io::ip_address>;

template <typename T>
concept iptypesnocvrefimpl = ::fast_io::details::inaddrnocvrefimpl<T> || ::std::same_as<T, ::fast_io::ipv4> ||
                             ::std::same_as<T, ::fast_io::ipv6> || ::std::same_as<T, ::fast_io::ip>;
...
[[msvc::forceinline]]
#endif
inline constexpr ::fast_io::manipulators::ip_manip_t<::fast_io::details::inaddrimpl<iptype>
                                                         ? ::fast_io::manipulators::ip_default_inaddr_flags
                                                         : ::fast_io::manipulators::ip_default_flags,
                                                     ::std::remove_cvref_t<iptype>>
print_alias_define(io_alias_t, iptype t) noexcept
{
    return {t};
}
...
inline constexpr ::std::size_t print_reserve_size(
    ::fast_io::io_reserve_type_t<char_type, ::fast_io::manipulators::ip_manip_t<flags, iptype>>) noexcept
{
    ...
    constexpr ::std::size_t v4sizewithport{v4totalsize + portsize + 1};

    // constexpr
    //	::std::size_t prefixlength{v4intsize+portsize+1};

    if constexpr (::std::same_as<nocvreftype, ::fast_io::posix_in_addr> || ::std::same_as<nocvreftype, ::fast_io::ipv4>)
    {
    ...
    {
        constexpr ::std::size_t v6intsize{portsize};

        constexpr ::std::size_t v6totalsize{(v6intsize + 1) * 8 - 1};

        constexpr ::std::size_t v6totalsizebracket{v6totalsize + 2};

        constexpr ::std::size_t v6sizewithport{v6totalsizebracket + 2 + portsize};
...
print_reserve_define(::fast_io::io_reserve_type_t<char_type, ::fast_io::manipulators::ip_manip_t<flags, iptype>>,
                     char_type *iter, ::fast_io::manipulators::ip_manip_t<flags, iptype> val) noexcept
{
    // using nocvreftype = ::std::remove_cvref_t<iptype>;
