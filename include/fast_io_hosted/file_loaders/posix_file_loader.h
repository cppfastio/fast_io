...
{
	if(file_size==0)
		return reinterpret_cast<char*>(static_cast<::std::intptr_t>(-1));
	return reinterpret_cast<char*>(sys_mmap(options.addr,file_size,options.prot,options.flags,fd,0));
}
...
inline char* posix_load_address(int fd,::std::size_t file_size)
{
	if(file_size==0)
		return reinterpret_cast<char*>(static_cast<::std::intptr_t>(-1));
	return reinterpret_cast<char*>(sys_mmap(nullptr,file_size,PROT_READ|PROT_WRITE,MAP_PRIVATE
...
inline posix_file_loader_return_value_t posix_load_address_impl(int fd)
{
	...
	auto ed{add};
	if(ed!=reinterpret_cast<char*>(static_cast<::std::intptr_t>(-1)))
	{
		ed+=size;
	}
	return {add,ed};
...
inline posix_file_loader_return_value_t posix_load_address_options_impl(::fast_io::posix_mmap_options const& options, int fd)
{
	::std::size_t size{posix_loader_get_file_size(fd)};
...
public:
	...
	pointer address_end{};
	inline explicit constexpr posix_file_loader() noexcept : address_begin(reinterpret_cast<char*>(static_cast<::std::intptr_t>(-1))),address_end(reinterpret_cast<char*>(static_cast<::std::intptr_t>(-1))){}
	...
	inline explicit posix_file_loader(native_fs_dirent fsdirent,open_mode om = open_mode::in, perms pm=static_cast<perms>(436))
	{
	...
	inline explicit posix_file_loader(T const& filename,open_mode om = open_mode::in,perms pm=static_cast<perms>(436))
	{
	...
	template<::fast_io::constructible_to_os_c_str T>
	inline explicit posix_file_loader(native_at_entry ent,T const& filename,open_mode om = open_mode::in,perms pm=static_cast<perms>(436))
	{
	...
	inline explicit posix_file_loader(posix_mmap_options const& options, native_fs_dirent fsdirent,open_mode om = open_mode::in, perms pm=static_cast<perms>(436))
	{
	...
	template<::fast_io::constructible_to_os_c_str T>
	inline explicit posix_file_loader(posix_mmap_options const& options, T const& filename,open_mode om = open_mode::in,perms pm=static_cast<perms>(436))
	{
	...
	template<::fast_io::constructible_to_os_c_str T>
	inline explicit posix_file_loader(posix_mmap_options const& options, native_at_entry ent,T const& filename,open_mode om = open_mode::in,perms pm=static_cast<perms>(436))
	{
	...
	{
		other.address_end=other.address_begin=reinterpret_cast<char*>(static_cast<::std::intptr_t>(-1));
	}
	...
	{
		::fast_io::details::posix_unload_address(address_begin,static_cast<::std::size_t>(address_end-address_begin));
		address_end=address_begin=reinterpret_cast<char*>(static_cast<::std::intptr_t>(-1));
	}
