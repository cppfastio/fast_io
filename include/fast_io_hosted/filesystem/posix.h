...
class posix_directory_io_observer
{
  public:
    ...
    template <::std::integral char_type> operator basic_posix_io_observer<char_type>() const noexcept
    {
...
template <> struct is_trivially_relocatable<posix_directory_file>
{
    inline static constexpr bool value = true;
};

template <> struct is_zero_default_constructible<posix_directory_file>
{
    inline static constexpr bool value = true;
};
...
struct posix_directory_entry
{
    ...
    struct dirent *entry{};
    ::std::size_t d_namlen{};
    template <::std::integral ch_type> explicit operator basic_posix_io_observer<ch_type>() const noexcept
    {
        return {details::dirp_to_fd(dirp)};
    }
};
...
template <typename StackType> struct basic_posix_recursive_directory_iterator
{
...
template <typename StackType> struct basic_posix_recursive_directory_generator
{
    using stack_type = StackType;
    posix_directory_file dir_fl;
};
...
template <typename StackType> inline void pop(basic_posix_recursive_directory_iterator<StackType> &prdit) noexcept
{
    if (prdit.stack.empty())
    {
        prdit.entry = nullptr;
