...
template <typename Allocator, typename T> struct nt_dirent_space_guard
{
    using pointer = T *;
    pointer ptr{};
...
template <nt_family family> inline nt_dirent *set_nt_dirent(nt_dirent *entry, bool start)
{
    ...
    entry->u8d_namlen = static_cast<::std::size_t>(
        ::fast_io::details::codecvt::general_code_cvt_full(enstart, enend, entry->u8d_name) - entry->u8d_name);
    ...
    __islink(wname, buffer)) data->entries[data->index].d_type = DT_LNK; else if ((w32fd.dwFileAttributes &
    FILE_ATTRIBUTE_DEVICE) == FILE_ATTRIBUTE_DEVICE) data->entries[data->index].d_type = DT_CHR; else if
    ((w32fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == FILE_ATTRIBUTE_DIRECTORY) data->entries[data->index].d_type
...
template <nt_family family> inline nt_dirent *set_nt_dirent_first(nt_dirent *entry)
{
    return set_nt_dirent<family>(entry, true);
}

template <nt_family family> inline nt_dirent *nt_dirent_next(nt_dirent *entry)
{
...
inline constexpr ::fast_io::manipulators::basic_os_c_str_with_known_size<char16_t>
native_filename(nt_directory_entry pioe) noexcept
{
    auto &ent{*pioe.entry};
...
u8filename(nt_directory_entry pioe) noexcept
{
    auto &ent{*pioe.entry};
...
template <nt_family family> struct nt_family_directory_iterator
{
    nt_dirent *entry{};
};

template <nt_family family> inline nt_directory_entry operator*(nt_family_directory_iterator<family> pdit) noexcept
{
    return {pdit.entry};
}
...
template <nt_family family, typename Allocator> struct basic_nt_family_directory_generator
{
...
template <nt_family family>
using nt_family_directory_generator =
    basic_nt_family_directory_generator<family, typename nt_api_encoding_converter::allocator_type>;
...
template <nt_family family, typename StackType> struct basic_nt_family_recursive_directory_iterator
{
...
template <nt_family family, typename Allocator, typename StackType> struct basic_nt_family_recursive_directory_generator
{
    using allocator_type = Allocator;
    using stack_type = StackType;
    void *root_handle{};
...
operator++(basic_nt_family_recursive_directory_iterator<family, StackType> &prdit)
{
    for (;;)
    {
        ...
        if (prdit.entry->d_type == file_type::directory)
        {
            ...
            if ((native_d_namlen == 1 && *native_d_name_ptr == u'.') ||
                (native_d_namlen == 2 && *native_d_name_ptr == u'.' && native_d_name_ptr[1] == u'.'))
                continue;
            prdit.stack.emplace_back(
                nt_at_entry{prdit.stack.empty() ? prdit.root_handle : prdit.stack.back().handle},
...
end(basic_nt_family_recursive_directory_generator<family, AllocatorType, StackType> const &) noexcept
{
    return {};
}

template <nt_family family, typename StackType>
inline nt_directory_entry
operator*(basic_nt_family_recursive_directory_iterator<family, StackType> const &prdit) noexcept
{
    return {prdit.entry};
}
...
inline bool operator==(::std::default_sentinel_t,
                       basic_nt_family_recursive_directory_iterator<family, StackType> const &b) noexcept
{
    return b.stack.empty() && b.entry == nullptr;
}

template <nt_family family, typename StackType>
inline bool operator==(basic_nt_family_recursive_directory_iterator<family, StackType> const &b,
...
template <nt_family family>
using nt_family_recursive_directory_generator = basic_nt_family_recursive_directory_generator<
    family, typename nt_directory_generator::allocator_type,
