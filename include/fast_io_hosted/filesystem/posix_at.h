...
template <posix_api_12 dsp> inline auto posix12_api_dispatcher(char const *oldpath, int newdirfd, char const *newpath)
{
...
#endif
    struct timespec
    unix_timestamp_to_struct_timespec64(unix_timestamp_option opt [[maybe_unused]]) noexcept
{
#if defined(UTIME_NOW) && defined(UTIME_OMIT)
    switch (opt.flags)
    {
...
inline auto posix_deal_with22(int olddirfd, old_path_type const &oldpath, int newdirfd, new_path_type const &newpath)
{
    return fast_io::posix_api_common(
        ...
        {
            return fast_io::posix_api_common(
                newpath, [&](char const *newpath_c_str)
