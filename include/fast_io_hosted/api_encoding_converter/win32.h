...
inline auto nt_api_common(T const &t, Func callback)
{
	...
	if constexpr (::std::is_array_v<T>)
	{
		...
		{
			return ::fast_io::details::nt_api_common_code_cvt_impl<allocator_type>(
	...
	else if constexpr (type_has_c_str_method<T>)
	{
		...
		{
			return ::fast_io::details::nt_api_common_code_cvt_impl<allocator_type>(::std::ranges::data(t),
																				   ::std::ranges::size(t), callback);
		}
		else if constexpr (::fast_io::details::cxx_std_filesystem_pseudo_concept<::std::remove_cvref_t<T>>)
		{
			auto const &native{t.native()};
			return ::fast_io::details::nt_api_common_code_cvt_impl<allocator_type>(native.c_str(), native.size(),
																				   callback);
		}
		else
		{
			auto ptr{t.c_str()};
			return ::fast_io::details::nt_api_common_code_cvt_impl<allocator_type>(ptr, ::fast_io::cstr_len(ptr),
																				   callback);
		}
	}
	else // types like ::std::basic_string_view, we must allocate new space to hold that type
	{
		...
		{
			return ::fast_io::details::api_common_copy_append0_strvw_case_impl<allocator_type, char16_t>(
				reinterpret_cast<char16_const_may_alias_ptr>(t.data()), t.length(), callback);
		}
		else
		{
