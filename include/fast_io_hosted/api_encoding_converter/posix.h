...
template <typename Allocator> struct basic_posix_api_encoding_converter
{
    ...
    constexpr
        ...
        : buffer_capacity(details::cal_decorated_reserve_size<sizeof(from_char_type), sizeof(char_type)>(sz)),
          buffer_data(details::allocate_iobuf_space<char_type, allocator_type>(
              ::fast_io::details::intrinsics::add_or_overflow_die(buffer_capacity, 1))),
...
using posix_api_encoding_converter =
    basic_posix_api_encoding_converter<generic_allocator_adapter<::fast_io::c_malloc_allocator>>;

namespace details
{
template <typename allocator_type = typename posix_api_encoding_converter::allocator_type, ::std::integral char_type,
          typename Func>
...
inline auto posix_api_common(T const &t, Func callback)
{
    ...
    if constexpr (::std::is_array_v<T>)
    {
        ...
        {
            return ::fast_io::details::posix_api_common_codecvt_impl<allocator_type>(
                p, ::fast_io::details::cal_array_size(t), callback);
        }
    }
    else if constexpr (type_has_c_str_method<T>)
    {
        ...
        else
        {
            ...
            {
                return ::fast_io::details::posix_api_common_codecvt_impl<allocator_type>(
                    ::std::ranges::data(t), ::std::ranges::size(t), callback);
            }
            else if constexpr (::fast_io::details::cxx_std_filesystem_pseudo_concept<::std::remove_cvref_t<T>>)
            {
                auto const &native{t.native()};
                return ::fast_io::details::posix_api_common_codecvt_impl<allocator_type>(native.c_str(), native.size(),
                                                                                         callback);
            }
            else
            {
                auto ptr{t.c_str()};
                return ::fast_io::details::posix_api_common_codecvt_impl<allocator_type>(ptr, ::fast_io::cstr_len(ptr),
                                                                                         callback);
            }
        }
    }
    else // types like ::std::basic_string_view, we must allocate new space to hold that type
    {
        ...
        {
            return ::fast_io::details::api_common_copy_append0_strvw_case_impl<allocator_type, char>(
                reinterpret_cast<char const *>(t.data()), t.length(), callback);
        }
        else
        {
