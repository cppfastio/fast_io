...
inline void io_async_wait(win32_io_observer iocp)
{
	...
	if (!GetQueuedCompletionStatus(iocp.native_handle(), __builtin_addressof(transferred),
								   __builtin_addressof(completionkey), __builtin_addressof(over),
...
inline bool iocp_io_async_wait_timeout_detail(win32_io_observer iocp, ::std::uint_least32_t millseconds)
{
	...
	win32::overlapped *over{};
	if (!GetQueuedCompletionStatus(iocp.native_handle(), __builtin_addressof(transferred),
								   __builtin_addressof(completionkey), __builtin_addressof(over), millseconds))
	{
