...
namespace details
{
inline ::fast_io::uintfpos_t posix_seek_impl(int fd,::fast_io::intfpos_t offset,seekdir s)
{
	...
	if(ret==-1)
		throw_posix_error();
	return static_cast<::fast_io::uintfpos_t>(static_cast<my_make_unsigned_t<off_t>>(ret));
#elif defined(__linux__)
#if defined(__NR_llseek)
	if constexpr(sizeof(off_t)<=sizeof(std::int_least32_t))
	{
		std::uint_least64_t result{};
		std::uint_least64_t offset64_val{static_cast<std::uint_least64_t>(static_cast<::fast_io::uintfpos_t>(offset))};
		...
		return static_cast<::fast_io::uintfpos_t>(static_cast<std::uint_least64_t>(result));
	}
	else
#endif
	{
		...
		system_call_throw_error(ret);
		return static_cast<::fast_io::uintfpos_t>(static_cast<std::uint_least64_t>(ret));
	}
#else
	if constexpr(sizeof(off_t)<=sizeof(std::int_least32_t))
	{
...
inline ::fast_io::uintfpos_t io_stream_seek_bytes_define(::fast_io::basic_posix_io_observer<char_type> bpiob,
