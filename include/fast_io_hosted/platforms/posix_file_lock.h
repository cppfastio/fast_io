...
inline void posix_file_lock_lock_impl(int fd, basic_flock_request<int_type> const &__restrict t)
{
    ...
    if constexpr (sizeof(flock_off_type) < sizeof(int_type))
    {
        ...
        if (t.start < flock_off_min || flock_off_max < t.start || t.len < flock_off_min || flock_off_max < t.len)
        {
...
{
    int fd{-1};
    template <::std::integral int_type> inline void lock(basic_flock_request<int_type> const &__restrict t)
    {
        ::fast_io::details::posix_file_lock_lock_impl(this->fd, t);
    }
    template <::std::integral int_type> inline void unlock(basic_flock_request<int_type> const &__restrict t) noexcept
    {
        ::fast_io::details::posix_file_lock_unlock_impl(this->fd, t);
    }
    template <::std::integral int_type> inline void try_lock(basic_flock_request<int_type> const &__restrict t)
    {
        ::fast_io::details::posix_file_lock_lock_impl(this->fd, t);
    }
};
