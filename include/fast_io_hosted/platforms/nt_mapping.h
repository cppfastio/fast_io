...
inline constexpr nt_file_map_attribute& operator&=(nt_file_map_attribute& x, nt_file_map_attribute y) noexcept{return x=x&y;}
...
public:
	...
	constexpr nt_family_memory_map_file(nt_family_memory_map_file&& __restrict other) noexcept:address_begin{other.address_begin},address_end{other.address_end}
	{
	...
	nt_family_memory_map_file& operator=(nt_family_memory_map_file&& __restrict other) noexcept
	{
		...
		{
			void* current_process_handle{reinterpret_cast<void*>(-1)};
			::fast_io::win32::nt::nt_unmap_view_of_section<family == ::fast_io::nt_family::zw>(current_process_handle, this->address_begin);
		}
