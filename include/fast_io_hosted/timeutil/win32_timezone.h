...
inline constexpr win32_timezone_name find_win32_regtz_position_impl(T const *str, ::std::size_t len) noexcept
{
    ...
    auto it{::fast_io::freestanding::lower_bound(
        first, last, scatter, [](::fast_io::basic_io_scatter_t<T> const &a, ::fast_io::basic_io_scatter_t<T> const &b)
...
inline win32_timezone_name win32_localtimezone_impl() noexcept
{
    ...
    else
    {
        ...
        {
            win32retcode = ::fast_io::win32::RegOpenKeyA(0x80000002 /*HKEY_LOCAL_MACHINE*/,
                                                         u8"SYSTEM\\CurrentControlSet\\Control\\TimeZoneInformation",
                                                         __builtin_addressof(k));
        }
        else
        {
            win32retcode = ::fast_io::win32::RegOpenKeyW(0x80000002 /*HKEY_LOCAL_MACHINE*/,
        ...
        ::std::uint_least32_t cbdata{bfbytes};
        {
            ...
            if constexpr (family == win32_family::ansi_9x)
            {
                win32retcode = ::fast_io::win32::RegQueryValueExA(k, u8"TimeZoneKeyName", nullptr, nullptr, buffer,
                                                                  __builtin_addressof(cbdata));
            }
            else
            {
                win32retcode = ::fast_io::win32::RegQueryValueExW(k, u"TimeZoneKeyName", nullptr, nullptr, buffer,
                                                                  __builtin_addressof(cbdata));
            }
            if (win32retcode)
            {
                ...
                if constexpr (family == win32_family::ansi_9x)
                {
                    win32retcode = ::fast_io::win32::RegQueryValueExA(k, u8"StandardName", nullptr, nullptr, buffer,
                                                                      __builtin_addressof(cbdata));
                }
                else
                {
...
template <win32_timezone_family tzfamily, win32_family family> struct basic_win32_family_local_timezone
{
...
inline constexpr basic_io_scatter_t<char_type> status_io_print_forward(io_alias_type_t<char_type>,
                                                                       win32_timezone_name tzn) noexcept
{
