...
template <typename T, typename mutex_type = native_mutex> using basic_iomutex = basic_general_iomutex<T, mutex_type>;

template <typename T, typename Mutex> struct basic_general_iomutex_ref
{
...
inline constexpr basic_general_iomutex_ref<T, Mutex>
output_stream_ref_define(basic_general_iomutex<T, Mutex> &m) noexcept
{
...
inline constexpr basic_general_iomutex_ref<T, Mutex>
input_stream_ref_define(basic_general_iomutex<T, Mutex> &m) noexcept
{
    return {__builtin_addressof(m)};
}
...
template <typename Mutex> struct basic_mutex_ref
{
    using mutex_type = Mutex;
