...
inline constexpr will_fail_t operator|(will_fail_t const& lhs, will_fail_t const& rhs) {
	if (lhs == will_fail_t::never) return rhs;
	else if (lhs == will_fail_t::compiletime) return will_fail_t::compiletime;
	else {
		if (rhs == will_fail_t::compiletime) return will_fail_t::compiletime;
		else return will_fail_t::runtime;
	}
}
...
void parse_prop_files(fast_io::native_file_loader&& file, file_property_t& file_properties) {
...
#endif
		if (linevw.empty()) continue;
		if (linevw.front() == u8'[') {
			if (linevw.back() != u8']') panic("invalid property file. unclosed bracket for sections");
			...
			if (section_name.front() == u8'"') {
				if (section_name.back() != u8'"') panic("invalid property file. unclosed quote for section name");
...
#endif
			if (!res) panic("invalid property file. duplicate section");
			curr_entry = &itr->second;
		} else {
			if (!curr_entry) panic("invalid property file. gobal property entry not allowed");
			auto eq_pos{linevw.find(u8'=')};
			if (eq_pos == std::u8string_view::npos) panic("invalid property file. entry should be in form of key-value pair");
...
#endif
			if (key.front() == u8'"') {
				if (key.back() != u8'"') panic("invalid property file. unclosed quote for key");
				key = key.substr(1, key.size() - 2);
			}
			if (key.empty()) panic("invalid property file. key cannot be empty");
			if (value.empty()) panic("invalid property file. value cannot be empty");
#if defined(__cpp_lib_associative_heterogeneous_insertion) && __cpp_lib_associative_heterogeneous_insertion >= 202311L
	...
	for (auto& [filename, entry] : file_contents) {
		...
		if (auto itr{entry.find(u8"ignore")}; itr != entry.end()) {
			if (itr->second == u8"true") file.ignore = true;
			else if (itr->second == u8"false") file.ignore = false;
			else panic("invalid property file. `ignore` should be of boolean type");
		}
		if (auto itr{entry.find(u8"interactive")}; itr != entry.end()) {
			if (itr->second == u8"true") file.interactive = true;
			else if (itr->second == u8"false") file.interactive = false;
			else panic("invalid property file. `interactive` should be of boolean type");
		}
		if (auto itr{entry.find(u8"asan")}; itr != entry.end()) {
			if (itr->second == u8"true") file.asan = true;
			else if (itr->second == u8"false") file.asan = false;
			else panic("invalid property file. `asan` should be of boolean type");
		}
		if (auto itr{entry.find(u8"will_fail")}; itr != entry.end()) {
			if (itr->second == u8"\"never\"") file.will_fail = will_fail_t::never;
			else if (itr->second == u8"\"compiletime\"") file.will_fail = will_fail_t::compiletime;
			else if (itr->second == u8"\"runtime\"") file.will_fail = will_fail_t::runtime;
			else panic("invalid property file. `will_fail` should be one of never, compiletime or runtime");
		}
		if (auto itr{entry.find(u8"platform")}; itr != entry.end()) {
			if (itr->second == u8"\"all\"") file.platform = platform_t::all;
			else if (itr->second == u8"\"windows\"") file.platform = platform_t::windows;
			else if (itr->second == u8"\"linux\"") file.platform = platform_t::linux;
			else if (itr->second == u8"\"macos\"") file.platform = platform_t::macos;
			else panic("invalid property file. `platform` should be one of all, windows, linux or macos");
		}
	}
}

bool gen_cmake_file(fast_io::native_io_observer nio, std::u8string_view prefix, file_entry_t prop);

void cmake_gen_rec_dir_def(fast_io::u8obuf_file& cmake_file, fast_io::native_io_observer nio, std::u8string_view filename, std::u8string_view prefix, file_entry_t const& prop) {
	if (prop.ignore) return;
	std::u8string new_prefix{fast_io::u8concat_std(prefix, filename, u8".")};
	fast_io::dir_file dir{at(nio), filename};
	if (!gen_cmake_file(dir, new_prefix, prop)) return;
	print(cmake_file, u8"include(${CMAKE_CURRENT_LIST_DIR}/", filename, u8"/CMakeLists.txt)\n");
}

void cmake_gen_file_def(fast_io::u8obuf_file& cmake_file, std::u8string_view targetname, auto&& filename, file_entry_t const& prop) {
	if (prop.ignore || prop.will_fail == will_fail_t::compiletime) return;
	if (prop.platform != platform_t::all && !intersects(prop.platform, global_platform)) return;
	print(cmake_file, u8"add_executable(", targetname, u8" ${CMAKE_CURRENT_LIST_DIR}/", filename, u8")\n");
	if (prop.interactive) return;
	if (prop.will_fail == will_fail_t::runtime) return;
	if (prop.asan) {
		if (msvc)
...
int main(int argc, char* argv[]) {
	...
	if (argc != 2 && argc != 3) {
		panic("Usage: ", fast_io::mnp::os_c_str(argv[0]), " <TARGET> <COMMAND-LINE-STYLE?>.\n"
